...
- **Content-Based Filtering (CBF)** – Recommends products based on _product features_ like categories and tags.
...
### 1. `backend/home/views.py` – ⚙️ **Core Logic for CF/CBF Processing**

This file processes recommendations using both CF:

| Function Name                                     | What It Does                                                                    |
| ------------------------------------------------- | ------------------------------------------------------------------------------- |
| `process_content_based_filtering()`               | Uses product categories/tags to build feature vectors and compute similarities. |
| `generate_user_recommendations_after_order(user)` | Generates final recommendations per user based on selected algorithm.           |

### 2. `backend/home/signals.py` – 🔁 **Triggers CF/CBF After Order**
...
- Switch between CF via radio buttons.
...
| --------------------------- | --------------------------------------------------------------- |
| `ProductSimilarity`         | Stores similarity scores for each product pair (type: CBF).     |
...
2. Selects filtering method: CBF
