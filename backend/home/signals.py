...
def handle_sentiment_analysis(sender, instance, created, **kwargs):
    if instance.content:
        ...
        sentiment_score, sentiment_category = sentiment_analyzer.analyze_sentiment(instance.content)
        ...
        if not getattr(instance, '_skip_sentiment_update', False):
...
def generate_association_rules_after_order(order):
    try:
        orders = Order.objects.prefetch_related('orderproduct_set__product').all()[:1000]
        transactions = []
