...
\item System automatycznie wywołuje endpoint \texttt{GET /api/recommendations/content-based/?product\_id=\{id\}}
...
\item System wywołuje endpoint \texttt{POST /api/admin/content-based/generate}
\item Backend uruchamia algorytm \texttt{ContentBasedRecommendationEngine.generate\_similarities()}
...
\item System wywołuje endpoint \texttt{GET /api/admin/content-based/debug/?product\_id=\{id\}}
...
\item System wywołuje endpoint \texttt{GET /api/fuzzy-logic-recommendations/?limit=10}
\item Backend wywołuje \texttt{FuzzyLogicAPI} z parametrem \texttt{user\_id} (z tokenu JWT)
\item System inicjalizuje silnik rozmyty \texttt{FuzzyEngine.initialize\_fuzzy\_system(user\_id)}
...
\item System wywołuje endpoint \texttt{GET /api/search/fuzzy/?query=loptap\&threshold=0.3}
...
\item Administrator klika „Analyze Product"
\item System wywołuje endpoint \texttt{GET /api/admin/fuzzy-debug/?product\_id=\{id\}}
...
\item System wywołuje endpoint \texttt{GET /api/probabilistic-recommendations/?user\_id=\{id\}}
\item Backend wywołuje \texttt{ProbabilisticAPI} z parametrem \texttt{user\_id}
\item System pobiera historię zamówień użytkownika z bazy danych wywołując \texttt{get\_user\_orders(user\_id)}
\item System ekstraktuje sekwencję kategorii produktów z zamówień uporządkowanych chronologicznie
\item System wywołuje \texttt{MarkovChain.predict\_next\_products(order\_sequence)}:
  \begin{itemize}
  ...
  \end{itemize}
\item System wywołuje \texttt{NaiveBayes.predict\_purchase\_probability(user\_profile)}:
  \begin{itemize}
...
\item System wywołuje endpoint \texttt{POST /api/admin/probabilistic/train-markov}
...
\item System wywołuje endpoint \texttt{POST /api/admin/probabilistic/train-bayes}
