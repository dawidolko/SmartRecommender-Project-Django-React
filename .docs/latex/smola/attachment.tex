...
\item System automatycznie wywołuje endpoint \texttt{GET /api/recommendations/\allowbreak content-based/?product\_id=\{id\}}
\item Backend wywołuje metodę \texttt{ContentBasedAPI} z parametrem \texttt{product\_id}
\item System wykonuje zapytanie do tabeli \texttt{ProductSimilarity} filtrując po \texttt{product1\_id = \{id\}} oraz \texttt{type='content\_based'}
...
\item System wywołuje endpoint \texttt{POST /api/admin/\allowbreak content-based/generate}
\item Backend uruchamia algorytm \texttt{ContentBased\allowbreak RecommendationEngine.\allowbreak generate\_similarities()}
...
\item System wywołuje endpoint \texttt{GET /api/admin/\allowbreak content-based/debug/?product\_id=\{id\}}
...
\item System wywołuje endpoint \texttt{GET /api/fuzzy-logic-\allowbreak recommendations/?limit=10}
\item Backend wywołuje \texttt{FuzzyLogicAPI} z parametrem \texttt{user\_id} (z tokenu JWT)
\item System inicjalizuje silnik rozmyty \texttt{FuzzyEngine.\allowbreak initialize\_fuzzy\_system(user\_id)}
...
\item System wywołuje endpoint \texttt{GET /api/search/fuzzy/\allowbreak ?query=loptap\&threshold=0.3}
...
\item Administrator klika „Analyze Product"
\item System wywołuje endpoint \texttt{GET /api/admin/\allowbreak fuzzy-debug/?product\_id=\{id\}}
...
\item System wywołuje endpoint \texttt{GET /api/probabilistic-\allowbreak recommendations/?user\_id=\{id\}}
\item Backend wywołuje \texttt{ProbabilisticAPI} z parametrem \texttt{user\_id}
\item System pobiera historię zamówień użytkownika z bazy danych wywołując \texttt{get\_user\_orders(\allowbreak user\_id)}
\item System ekstraktuje sekwencję kategorii produktów z zamówień uporządkowanych chronologicznie
\item System wywołuje \texttt{MarkovChain.\allowbreak predict\_next\_products(order\_sequence)}:
  \begin{itemize}
  ...
  \end{itemize}
\item System wywołuje \texttt{NaiveBayes.\allowbreak predict\_purchase\_probability(user\_profile)}:
  \begin{itemize}
...
\item System wywołuje endpoint \texttt{POST /api/admin/\allowbreak probabilistic/train-markov}
\item Backend uruchamia \texttt{MarkovChainEngine.train()}:
  \begin{itemize}
...
\item System wywołuje endpoint \texttt{POST /api/admin/\allowbreak probabilistic/train-bayes}
\item Backend uruchamia \texttt{NaiveBayesEngine.train()}:
  \begin{itemize}
...
\noindent
\textbf{Warunki końcowe:} Modele probabilistyczne (Markov Chain i Naive Bayes) zostały wytrenowane na aktualnych danych, rekomendacje są aktualne

\subsection*{Scenariusz 22: Debugowanie modeli probabilistycznych}

\noindent
\textbf{Aktor:} Administrator

\noindent
\textbf{Warunki początkowe:} Administrator jest zalogowany, modele zostały wytrenowane

\noindent
\textbf{Przebieg scenariusza głównego - Markov Chain:}
\begin{enumerate}
\item Administrator otwiera panel administracyjny
\item Administrator klika sekcję „Debug"
\item Administrator wybiera zakładkę „Markov Chain Debug"
\item System wyświetla panel debugowania zawierający:
  \begin{itemize}
  \item Wzór matematyczny macierzy przejść z wygładzaniem Laplace'a
  \item Statystyki: liczba stanów (48 kategorii), łączna liczba przejść, wygładzanie $\alpha$ = 1.0, timestamp ostatniego treningu
  \item Wizualizację macierzy przejść jako heatmap 48x48
  \end{itemize}
\item Administrator wybiera kategorię źródłową z listy rozwijanej (np. „Laptopy")
\item System wyświetla Top 10 kategorii docelowych z najwyższymi prawdopodobieństwami przejścia:
  \begin{itemize}
  \item Akcesoria komputerowe: 0.45 (45\%)
  \item Peryferia: 0.30 (30\%)
  \item Torby i plecaki: 0.15 (15\%)
  \item Oprogramowanie: 0.05 (5\%)
  \item Inne kategorie: <0.05 (łącznie 5\%)
  \end{itemize}
\item Administrator klika „Show Transition Examples"
\item System pobiera przykładowe rzeczywiste sekwencje zamówień użytkowników, które wygenerowały te przejścia
\item Administrator weryfikuje sensowność przejść
\item Administrator przełącza zakładkę na „Naive Bayes Debug"
\item System wyświetla panel Naive Bayes zawierający:
  \begin{itemize}
  \item Wzór twierdzenia Bayesa
  \item Statystyki: liczba próbek treningowych, accuracy, precision, recall, F1-score
  \item Tabelę prawdopodobieństw warunkowych $P(feature | class)$ dla każdej z 5 cech
  \item Prawdopodobieństwa a priori: $P(active)$ = 0.65, $P(inactive)$ = 0.35
  \end{itemize}
\item Administrator wprowadza ID użytkownika w pole „User ID for prediction"
\item System pobiera profil użytkownika i wykonuje predykcję krok po kroku:
  \begin{itemize}
  \item Ekstraktuje cechy użytkownika
  \item Pokazuje obliczenia $P(active | features)$ z rozpisaniem każdego czynnika
  \item Wyświetla końcowe prawdopodobieństwo oraz klasyfikację (active/inactive)
  \end{itemize}
\item Administrator weryfikuje poprawność predykcji
\end{enumerate}
