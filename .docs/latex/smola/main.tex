...
\textbf{Warstwa danych} - relacyjna baza danych PostgreSQL 14:
...
\textbf{Progi dla jakości (rating 1-5):}
\begin{itemize}
...
\textbf{Progi dla popularności (liczba zamówień):}
\begin{itemize}
...
Parametry modeli probabilistycznych są definiowane w pliku \texttt{custom\_recommendation\_engine.py}:

\textbf{Parametry łańcucha Markowa (CustomMarkovChain):}
\begin{itemize}
...
\textbf{Parametry Naive Bayes (CustomNaiveBayes):}
\begin{itemize}
...
\textbf{Parametry integracji (ProbabilisticRecommendationEngine):}
\begin{itemize}
