...
\begin{figure}[h!]
  ...
  \caption{Strona główna - widok karty produktu z przyciskami akcji i informacjami o cenie.}
  \label{fig:main_view2}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\textwidth]{images/fuzzySearch1.jpg}
  \caption{Wyszukiwanie rozmyte - funkcjonalność pozwalająca na wyszukiwanie produktów z tolerancją na błędy i synonimy.}
...
\begin{figure}[h!]
  ...
  \caption{Panel statystyk klienta - historia zakupów i analiza preferencji zakupowych użytkownika.}
...
\begin{figure}[h!]
  ...
  \caption{Panel statystyk klienta - wykres rozkładu kategorii zakupowych i ulubione produkty.}
...
\begin{figure}[h!]
  ...
  \caption{Widok opinii o produkcie - lista recenzji klientów z ocenami wykorzystywanymi przez algorytmy rekomendacyjne.}
  \label{fig:opinion_view}
\end{figure}
...
Aplikacja jest skonteneryzowana z użyciem Docker i Docker Compose. Struktura kontenerów obejmuje:

\begin{itemize}
    \item \textbf{backend} — Django REST Framework z algorytmami rekomendacyjnymi
    \item \textbf{frontend} — React 18 z Vite jako bundler
    \item \textbf{db} — PostgreSQL 14 z danymi produktów i użytkowników
    \item \textbf{nginx} — reverse proxy dla produkcji
\end{itemize}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\textwidth]{images/dockerView.jpg}
  \caption{Docker Desktop - widok uruchomionych kontenerów aplikacji SmartRecommender.}
