...
\textbf{Warstwa logiki biznesowej (Backend)} — Serwer aplikacyjny Django 4.2 z Django REST Framework, implementujący trzy niezależne silniki rekomendacyjne oraz udostępniający REST API.

\textbf{Warstwa danych (Database)} — Relacyjna baza danych PostgreSQL 14 przechowująca dane produktów, użytkowników, zamówień oraz prekompilowane wyniki algorytmów (macierz podobieństw CBF, macierz przejść Markowa, profile rozmyte użytkowników).

System został zaprojektowany zgodnie z zasadą separacji odpowiedzialności (Separation of Concerns), gdzie każdy moduł ma jasno zdefiniowaną rolę i komunikuje się z innymi modułami przez dobrze zdefiniowane interfejsy.

\subsection*{3.2 Projekt modułów rekomendacyjnych}
\addcontentsline{toc}{subsection}{3.2 Projekt modułów rekomendacyjnych}

System składa się z trzech niezależnych modułów rekomendacyjnych, które mogą działać osobno lub być łączone w system hybrydowy:
...
System definiuje trzech aktorów z różnymi poziomami uprawnień oraz przypadki użycia związane z rekomendacjami produktów. Szczegółowy opis znajduje się w rozdziale 4 (Architektura techniczna systemu), natomiast tutaj przedstawiamy główne przypadki użycia z perspektywy projektu:

\textbf{Aktorzy:}
\begin{itemize}
    \item Gość (niezalogowany) — przegląda produkty, wyszukuje rozmyte, dodaje do koszyka
    \item Klient (zalogowany) — wszystkie uprawnienia Gościa + spersonalizowane rekomendacje + historia zamówień
    \item Administrator — zarządza algorytmami, debuguje, generuje rekomendacje globalnie
\end{itemize}

\textbf{Główne przypadki użycia:}
\begin{itemize}
    \item UC-01: Przeglądanie katalogu produktów (Gość, Klient, Admin)
    \item UC-02: Wyszukiwanie rozmyte produktów (Gość, Klient, Admin)
    \item UC-03: Wyświetlanie rekomendacji CBF (Klient, Admin)
    \item UC-04: Wyświetlanie rekomendacji Fuzzy (Klient, Admin)
    \item UC-05: Wyświetlanie rekomendacji Probabilistic (Klient, Admin)
    \item UC-06: Składanie zamówienia (Klient)
    \item UC-07: Zmiana aktywnego algorytmu rekomendacji (Admin)
    \item UC-08: Debugowanie algorytmów (podgląd wag, macierzy, reguł) (Admin)
    \item UC-09: Generowanie macierzy podobieństw CBF (Admin)
    \item UC-10: Trening modeli probabilistycznych (Admin)
\end{itemize}
...
\textbf{Moduł Produktów i Kategorii}:
Tabela produktów przechowuje nazwę, cenę, opis oraz relacje wiele-do-wielu z kategoriami i tagami. Specyfikacje techniczne zapisane w formacie klucz-wartość umożliwiają elastyczne definiowanie cech produktów wykorzystywanych przez algorytm CBF.

\textbf{Moduł Użytkowników i Zamówień}:
Rozszerzony model użytkownika Django przechowuje dane uwierzytelniające oraz role. Historia zamówień rejestruje transakcje użytkowników, stanowiąc podstawę dla wszystkich trzech metod rekomendacyjnych.

\textbf{Moduł Rekomendacji}:
Tabela podobieństw produktów (\texttt{ProductSimilarity}) przechowuje prekompilowane wartości podobieństwa kosinusowego. Profile rozmyte użytkowników (\texttt{FuzzyUserProfile}) agregują wrażliwość cenową i preferencje kategorialne w formacie JSON. Macierz przejść Markowa (\texttt{MarkovTransitions}) modeluje prawdopodobieństwa zakupów sekwencyjnych.
