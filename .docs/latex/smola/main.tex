...
Interpretacja: Produkty z oceną $\geq 4.5$ (maksymalna jakość) mają pełną przynależność do zbioru "high". Ocena 3.5 jest punktem środkowym - produkt należy w równym stopniu do zbiorów rozmytych o sąsiednich kategoriach jakości. Przejścia są płynne, co odzwierciedla niepewność i subiektywność w ocenach użytkowników - produkt z oceną 4.0 jest częściowo "średni"~i częściowo "wysoki".
...
\noindent Rysunek \ref{fig:cbf_debug1} przedstawia Panel zawierajacy następujące sekcje:

\begin{itemize}
    \item \textbf{Select Product To Analyze} - pole do wyboru produktu, dla którego zostanie przedstawiony szczegółowy debug
    \item \textbf{Algorithm}:
    \begin{itemize}
        \item Name: Content-Based Filtering (Cosine Similarity)
        \item Formula: wzór podobieństwa kosinusowego ważonego
    \end{itemize}
    \item \textbf{Database Statistics}:
    \begin{itemize}
        \item Total Products: 500
        \item Saved Similarities: 16038
        \item Percentage Saved: 6.43\%
        \item Threshold: 20\%
    \end{itemize}
    \item \textbf{Feature Weights}:
    \begin{itemize}
        \item Category: 40\%
        \item Tag: 30\%
        \item Price: 20\%
...
Panel wyświetla szczegółową analizę dla wybranego produktu w trzech sekcjach (Rysunek \ref{fig:cbf_debug2}):

\begin{itemize}
    \item \textbf{Selected Product}:
    \begin{itemize}
        \item Name: A4 Tech HD PK-910P USB Black
        \item ID: 295
        \item Price: 29.99 PLN
        \item Price Category: low
        \item Categories: peripherals.webcams
        \item Tags: Budget
        \item Keywords: video, quality, perfect, calls, built
    \end{itemize}
    \item \textbf{Feature Vector (8 features)} - wektor cech produktu z przypisanymi wagami:
    \begin{itemize}
        \item category\_peripherals.webcams: 0.400
        \item tag\_budget: 0.300
        \item price\_low: 0.200
        \item keywords\_video: 0.020
        \item keywords\_quality: 0.020
        \item keywords\_perfect: 0.020
        \item keywords\_calls: 0.020
        \item keywords\_built: 0.020
    \end{itemize}
    \item \textbf{Top 10 Similar Products} - tabela z najbardziej podobnymi produktami zawierająca kolumny: nazwa produktu, similarity score, cena, kategoria
\end{itemize}
...
\noindent Zakładka "Next Purchase (Markov)"~prezentuje:
\begin{itemize}
...
\noindent Zakładka ``Behavior Insights (Bayesian)'' wykorzystuje model Naive Bayes do analizy preferencji zakupowych:
\begin{itemize}
...
Panel wyświetla następujące sekcje (Rysunek \ref{fig:prob_debug1}):
\begin{itemize}
    \item \textbf{User ID for Analysis} - pole do wyboru analizowanego użytkownika
    \item \textbf{Product ID for Analysis} - pole do wyboru analizowanego produktu
    \item \textbf{Algorithm Information}:
    \begin{itemize}
        \item Name: Probabilistic Models (Markov Chain + Naive Bayes)
        \item Description: System rekomendacji oparty na łańcuchach Markova i klasyfikatorze Naive Bayes
    \end{itemize}
    \item \textbf{Markov Chain Model}:
    \begin{itemize}
        \item Order: 1 (first-order Markov Chain)
        \item Number of States: 48 (kategorie produktowe)
        \item Total Transitions: 48
    \end{itemize}
    \item \textbf{Macierz Top 10 Transitions} -- tabela z 10 najczęstszymi przejściami między kategoriami produktowymi
\end{itemize}
...
Panel wyświetla szczegółową analizę w trzech sekcjach (Rysunek \ref{fig:prob_debug2}):

\begin{itemize}
    \item \textbf{Naive Bayes Purchase Prediction}:
    \begin{itemize}
        \item Trained: Yes
    ...
    \end{itemize}
    \item \textbf{Naive Bayes - Churn Prediction}:
    \begin{itemize}
        \item Trained: Yes
        \item Number of Features: 3
        \item Classes: will\_churn
        \item Class Priors: will\_churn = 1.0
    \end{itemize}
    \item \textbf{Product Analysis}:
    \begin{itemize}
        \item Product: A4Tech HD PK-910P USB Black
        \item ID: 295
        \item Category: peripherals.webcams
    \end{itemize}
        \item \textbf{Macierz Next Likely Categories} -- tabela z prawdopodobieństwami przejść do kolejnych kategorii produktowych dla wybranego produktu
    \end{itemize}
\end{itemize}
