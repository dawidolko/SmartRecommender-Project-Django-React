...
\begin{itemize}
    ...
    \item \textbf{Modele probabilistyczne} - przewidują sekwencje zakupowe i ryzyko churnu na podstawie historii transakcji
\end{itemize}
...
\item \textbf{Brak wieloźródłowej agregacji sentymentu, logiki rozmytej i modeli probabilistycznych} - Google Recommendations AI oferuje funkcję ``Frequently Bought Together'' (często kupowane razem, podobną do algorytmu Apriori), ale nie wspiera agregacji sentymentu z wielu źródeł tekstowych, systemów rozmytych z funkcjami przynależności ani łańcuchów Markova jak w planowanym systemie,
...
\item \textbf{Funkcje przynależności Mamdani} dla logiki rozmytej - modelowanie niepewności preferencji użytkowników,
...
\item Przeglądanie spersonalizowanych rekomendacji produktów:
  \begin{itemize}
\item \textit{Content-Based Filtering} - rekomendacje produktów podobnych na podstawie cech,
\item \textit{Fuzzy Logic} - rekomendacje z wykorzystaniem logiki rozmytej i profili użytkowników,
\item \textit{Modele probabilistyczne} - predykcja sekwencji zakupowych i prawdopodobieństwa zakupu.
\end{itemize}
\end{itemize}
...
\item Wyświetlanie statystyk i metryk,
\end{itemize}
\item Generowanie macierzy podobieństw (CBF),
\item Trening modeli probabilistycznych,
...
\begin{itemize}
\item \textbf{Statystyki zakupowe} - liczba zakupionych produktów, reklamacji, zamówień w bieżącym miesiącu, średnia wartość zamówienia,
\item \textbf{Wykresy analityczne} - trendy zamówień w czasie (wykres liniowy) oraz rozkład wydatków po kategoriach (wykres kołowy),
\item \textbf{Rekomendacje personalizowane} - sekcja z 4 produktami wygenerowanymi przez aktualnie aktywny algorytm wybrany w ustawieniach (CBF, Fuzzy Logic lub Probabilistic).
\end{itemize}

Użytkownik może przejść do dedykowanych zakładek z bardziej szczegółowymi rekomendacjami:
\begin{itemize}
\item \textit{Smart Recommendations} - zaawansowana analiza probabilistyczna (łańcuch Markowa, Naive Bayes) z wykresami rozkładu kategorii i predykcją następnych zakupów,
\item \textit{Fuzzy Logic} - rekomendacje z pełnym wyjaśnieniem reguł rozmytych, profilu użytkownika i aktywacji reguł IF-THEN.
\end{itemize}
...
\textbf{Scenariusz 3: Gość przegląda produkty}

Niezalogowany użytkownik przegląda katalog produktów, dodaje laptop do koszyka. Przy próbie finalizacji zamówienia system przekierowuje do logowania lub rejestracji.
...
\begin{itemize}
\item \textbf{Modułowość} - trzy niezależne algorytmy rekomendacji (CBF, Fuzzy Logic, Probabilistic), które mogą działać osobno lub równolegle w ramach wspólnej infrastruktury,
\item \textbf{Komplementarność metod} - każdy algorytm rozwiązuje inny aspekt problemu rekomendacji (zimny start, personalizacja, interpretowalność),
\item \textbf{Architektura trójwarstwowa} - separacja prezentacji (React 18), logiki biznesowej (Django 5.1.4 + DRF 3.15.2) oraz danych (PostgreSQL 14),
...
\item \textbf{recommendation\_views.py} -- endpointy CBF: \texttt{/api/content-based-debug/},\\
\texttt{/api/generate-similarities/},
\item \textbf{fuzzy\_views.py} -- endpointy Fuzzy: \texttt{/api/fuzzy-recommendations/},\\
\texttt{/api/fuzzy-debug/},
\item \textbf{probabilistic\_views.py} -- endpointy Markov i Bayes:\\
\texttt{/api/markov-recommendations/}, \texttt{/api/bayesian-insights/},
...
\item \textbf{SearchModal.jsx} -- zaawansowany modal wyszukiwania z fuzzy search (wyszukiwanie z wykorzystaniem logiki rozmytej),
...
\item \textbf{ProductPage.jsx} -- szczegółowy widok produktu z galerią zdjęć, opisem, specyfikacjami technicznymi, sekcją opinii,
\item \textbf{CartContent.jsx} -- koszyk zakupowy,
\item \textbf{ClientPanel.jsx} -- panel klienta z zakładkami: Dashboard, Orders, Account, Recommendations (CBF, Fuzzy Logic, Probabilistic),
\item \textbf{AdminPanel.jsx} -- panel administracyjny z narzędziami debugowania algorytmów ML.
\end{itemize}
...
\begin{itemize}
\item \textbf{Separacja kontynerów} - wyraźny podział frontend/backend/baza danych zapewnia modułowość i łatwość utrzymania,
\item \textbf{Wydajność implementacji} - narzędzia (Django ORM, React Hooks, Docker Compose) przyspieszają rozwój aplikacji bez utraty kontroli nad algorytmami,
\item \textbf{Wsparcie społeczności} - aktywna społeczność, obszerna dokumentacja oraz bogate ekosystemy bibliotek (Django REST Framework, React ecosystem),
\item \textbf{Optymalizacja wydajności} - cache, indeksy bazodanowe, operacje wektorowe NumPy oraz threshold pruning zapewniają szybki czas odpowiedzi nawet dla dużych katalogów produktów.
\end{itemize}
...
\subsubsection*{5.1.1 Architektura systemu CBF}
...
\subsubsection*{5.1.2 Implementacja ekstrakcji cech}
...
\subsubsection*{5.1.3 Algorytm podobieństwa kosinusowego}
...
\subsubsection*{5.1.4 Generowanie macierzy podobieństw}
...
Przepływ procesu generowania rekomendacji CBF:
\begin{enumerate}
    \item Żądanie użytkownika trafia do API\\
    ...
    \item System sprawdza cache - w przypadku trafienia zwracany jest wynik natychmiast
    \item Backend zwraca odpowiedź 200 OK z listą rekomendacji (product\_id, name, price, similarity\_score)
    \item Frontend wyświetla sekcję rekomendacji z podobnymi produktami
\end{enumerate}
...
\subsubsection*{5.2.1 Architektura systemu Fuzzy Logic}
...
\subsubsection*{5.2.2 Funkcje przynależności -- szczegóły implementacji}
...
\subsubsection*{5.2.3 Rozmyty profil użytkownika}
...
\subsubsection*{5.2.4 Baza reguł rozmytych}
...
\subsubsection*{5.2.5 Wnioskowanie i defuzzyfikacja}
...
Przepływ procesu generowania rekomendacji Fuzzy Logic:
\begin{enumerate}
    \item Żądanie użytkownika trafia do API (\texttt{/api/fuzzy-recommendations/})
    \item FuzzyLogicEngine buduje rozmyty profil użytkownika z historii zakupów (FuzzyUserProfile)
    \item Dla każdego produktu system przeprowadza fuzzyfikację (obliczenie przynależności price, quality, popularity)
    \item Następuje wnioskowanie - ewaluacja 6 reguł IF-THEN i obliczenie aktywacji
    \item Defuzzyfikacja agreguje wyniki reguł do jednego fuzzy\_score metodą średniej ważonej
    \item Backend zwraca produkty posortowane po fuzzy\_score wraz z rule\_activations dla transparentności
    \item System cache'uje wyniki na 5 minut dla optymalizacji wydajności
\end{enumerate}
...
\subsubsection*{5.3.1 Architektura systemu probabilistycznego}
...
\subsubsection*{5.3.2 Łańcuch Markowa dla sekwencji zakupowych}
...
\subsubsection*{5.3.3 Naiwny klasyfikator Bayesa}
...
\subsubsection*{5.3.4 Integracja modeli -- ProbabilisticRecommendationEngine}
...
\subsubsection*{5.3.5 API probabilistyczne}
...
Przepływ procesu generowania rekomendacji probabilistycznych:
\begin{enumerate}
    \item Żądanie użytkownika trafia do API (\texttt{/api/markov-recommendations/})
    \item System trenuje oba modele na aktualnych danych: CustomMarkovChain (macierz przejść między kategoriami) i CustomNaiveBayes (klasyfikatory purchase i churn)
    \item Markov Chain analizuje ostatnie zamówienie użytkownika i przewiduje następne kategorie zakupów z prawdopodobieństwami
    \item Naive Bayes oblicza prawdopodobieństwo zakupu (purchase\_probability) oraz ryzyko churnu dla użytkownika
    \item ProbabilisticRecommendationEngine agreguje wyniki obu modeli (60\% Markov, 40\% NB) i wybiera top K produktów
    \item Backend zwraca rekomendacje wraz z metadanymi: predicted\_categories, purchase\_probability, expected\_days\_to\_next\_order
    \item System zapisuje predykcje do tabeli PurchaseProbability dla późniejszej analizy trendów
\end{enumerate}
...
\subsection*{5.4 Podsumowanie implementacji algorytmów}
\addcontentsline{toc}{subsection}{5.4 Podsumowanie implementacji algorytmów}
...
\textbf{Modele probabilistyczne} -- łańcuch Markowa pierwszego rzędu (predykcja sekwencji zakupowych kategorii) oraz klasyfikator Naive Bayes (prawdopodobieństwo zakupu i churn). System wykorzystuje historię zamówień do budowy macierzy przejść i modeli predykcyjnych.
...
\begin{enumerate}
    \item \textbf{Zbieranie danych}: System gromadzi dane o działaniach użytkownika (przeglądanie produktów, dodawanie do koszyka, składanie zamówień) poprzez API Django REST Framework
    \item \textbf{Trening modeli}: Modele są trenowane automatycznie po każdej zmianie danych (nowe zamówienie, nowy produkt) lub manualnie przez administratora
    \item \textbf{Generowanie rekomendacji}: Na podstawie profilu użytkownika i danych historycznych system generuje rekomendacje przy użyciu jednej z trzech metod (CBF, Fuzzy Logic, Probabilistic)
    \item \textbf{Prezentacja wyników}: Rekomendacje są wyświetlane w interfejsie użytkownika (strona główna, strona produktu, panel klienta) oraz w panelu administracyjnym
\end{enumerate}
...
Panel administracyjny umożliwia dynamiczne przełączanie między metodami rekomendacji wyświetlanymi na stronie głównej aplikacji. Administrator może wybrać aktywny algorytm poprzez dedykowany interfejs konfiguracyjny, który będzie wykorzystywany we wszystkich sekcjach rekomendacji. System zawiera łącznie sześć metod rekomendacyjnych zaimplementowanych w ramach dwóch prac inżynierskich.

\medskip

\textbf{Zaimplementowane metody rekomendacyjne:}

\begin{itemize}
\item \textbf{Content-Based Filtering (CBF)} - rekomendacje oparte na podobieństwie cech produktów (kategoria, tagi, cena, słowa kluczowe). Rozwiązuje problem zimnego startu dla nowych produktów.
\item \textbf{Fuzzy Logic} - system rozmytego wnioskowania Mamdani z 6 regułami IF-THEN. Oferuje najwyższą interpretowalność i modeluje niepewność w preferencjach użytkownika.
\item \textbf{Probabilistic Methods (Markov + Naive Bayes)} - łańcuch Markowa przewiduje sekwencje zakupowe kategorii, Naive Bayes ocenia prawdopodobieństwo zakupu i churn. Najgłębsza personalizacja.
\end{itemize}

Zmiana metody następuje natychmiast po zapisaniu ustawień i wpływa na wszystkie sekcje rekomendacji w aplikacji. 

\subsection*{6.3 Content-Based Filtering w praktyce}
\addcontentsline{toc}{subsection}{6.3 Content-Based Filtering w praktyce}
...
\subsection*{6.4 Logika rozmyta w praktyce}
\addcontentsline{toc}{subsection}{6.4 Logika rozmyta w praktyce}
...
\subsection*{6.5 Modele probabilistyczne w praktyce}
\addcontentsline{toc}{subsection}{6.5 Modele probabilistyczne w praktyce}
...
\textbf{1. Cache'owanie wyników (Django Cache Framework)}

System wykorzystuje wbudowany mechanizm cache'owania Django z różnymi poziomami czasu wygaśnięcia:

\begin{itemize}
\item \textbf{CACHE\_TIMEOUT\_SHORT = 300s (5 minut)} - rekomendacje dla zalogowanych użytkowników, wyniki wyszukiwania rozmytego
...
\textbf{2. Bulk operations (operacje zbiorcze)}
...
Przykład: pobranie 100 produktów z kategoriami bez prefetchingu = 101 zapytań SQL (N+1 problem). Z \texttt{prefetch\_related('categories')} = 2 zapytania SQL.

\textbf{4. Threshold pruning (przycinanie progowe)}
...
\item Klucze obce (FOREIGN KEY) - przyspieszenie JOIN'ów
...
\textbf{Podsumowanie mechanizmów optymalizacji}

Kombinacja cache'owania, bulk operations, threshold pruning oraz indeksowania pozwala systemowi obsługiwać 500 produktów i generować rekomendacje w czasie rzeczywistym (<200ms) nawet przy braku cache. Dla większych katalogów (>10\,000 produktów) zalecane są dalsze optymalizacje: approximate nearest neighbors (LSH, HNSW), partycjonowanie tabel PostgreSQL, oraz rozproszenie obliczeń (Celery + Redis).
...
\begin{itemize}
    \item \textbf{Dla użytkowników}: Spersonalizowane rekomendacje produktów, analiza wzorców zakupowych, predykcje churn i prawdopodobieństwa zakupu
...
\section*{Rozdzia\l{} 7}
\addcontentsline{toc}{section}{Rozdział 7: Wyniki i ewaluacja}
...
\begin{itemize}
    ...
    \item Środowisko: Django 5.1.4, PostgreSQL 14, 16GB RAM, Intel i7 8750H
\end{itemize}
...
Redukcja rekordów & $\sim$97\% (4k z 125k par) \\
Bulk insert speedup & 80x szybciej \\
\hline
\end{tabular}
\caption{Wydajność Content-Based Filtering (wartości orientacyjne)}
\end{table}
...
\hline
\end{tabular}
\caption{Wydajność Fuzzy Logic (wartości orientacyjne)}
\end{table}
...
\hline
\end{tabular}
\caption{Wydajność modeli probabilistycznych (wartości orientacyjne)}
\end{table}
...
\hline
\end{tabular}
\caption{Porównanie jakościowe zaimplementowanych metod rekomendacji}
\end{table}
...
\subsection*{7.7 Komplementarność metod w systemie kompleksowym}
\addcontentsline{toc}{subsection}{7.7 Komplementarność metod w systemie kompleksowym}

Każda z zaimplementowanych metod adresuje inny aspekt problemu rekomendacji:

\textbf{Metody oparte na treści} (CBF): odkrywanie produktów podobnych na podstawie cech. Rozwiązuje problem zimnego startu dla nowych produktów.

\textbf{Metody behawioralne} (Fuzzy Logic, modele probabilistyczne): personalizacja na podstawie profilu użytkownika (Fuzzy) oraz sekwencji zakupowych (Markov). Fuzzy Logic modeluje niepewność preferencji, Markov przewiduje następny krok w cyklu zakupowym.
...
\subsection*{8.1 Ograniczenia systemu}
\addcontentsline{toc}{subsection}{8.1 Ograniczenia systemu}
...
\textbf{Skalowalność dla bardzo dużych katalogów} -- dla katalogów produktów przekraczających 10\,000 pozycji mogą wystąpić wyzwania wydajnościowe. Obecne optymalizacje (threshold pruning, cache, bulk operations) są wystarczające dla katalogów do 1\,000-2\,000 produktów, ale większe wymagałyby zastosowania approximate nearest neighbors (LSH, HNSW) lub partycjonowania tabel PostgreSQL.
...
\subsection*{8.2 Kierunki dalszego rozwoju}
\addcontentsline{toc}{subsection}{8.2 Kierunki dalszego rozwoju}
...
\textbf{Rekomendacje w czasie rzeczywistym} -- obecny system wykorzystuje cache z okresem ważności 5-120 minut. Implementacja systemu aktualizującego rekomendacje w czasie rzeczywistym po każdej akcji użytkownika (przeglądanie produktów, dodawanie do koszyka) mogłaby zwiększyć trafność sugestii.
...
\subsection*{8.3 Wnioski końcowe}
\addcontentsline{toc}{subsection}{8.3 Wnioski końcowe}
...
\textbf{Modele probabilistyczne} umożliwiają najgłębszą personalizację dla użytkowników z bogatą historią zakupów. Łańcuch Markowa przewiduje sekwencje zakupowe na poziomie kategorii produktów, Naive Bayes ocenia prawdopodobieństwo zakupu i ryzyko churnu.
...
Product Recommendation System Based on Content-Based Filtering, Fuzzy Logic, and Probabilistic Models
