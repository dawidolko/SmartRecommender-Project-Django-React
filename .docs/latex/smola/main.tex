...
JEZELI quality JEST high ORAZ price JEST cheap
...
\begin{figure}[h!]
  ...
  \caption{Panel administratora - system rekomendacji z wybraną metodą Content-Based Filtering.}
  \label{fig:cbf_admin}
\end{figure}
...
\begin{figure}[h!]
  ...
  \caption{Panel administratora - system rekomendacji z wybraną metodą Fuzzy Logic.}
...
\begin{figure}[h!]
  ...
  \caption{Panel klienta - zakładka ``Your Fuzzy User Profile'' z profilem rozmytym użytkownika: typ profilu, wrażliwość cenowa i ulubione kategorie.}
...
\subsection*{3.8 Wyszukiwanie rozmyte (Fuzzy Search)}
\addcontentsline{toc}{subsection}{3.8 Wyszukiwanie rozmyte (Fuzzy Search)}
...
\begin{figure}[h!]
  ...
  \caption{Wyszukiwarka rozmyta - funkcjonalność pozwalająca na wyszukiwanie produktów z tolerancją na błędy pisowni i literówki.}
...
\begin{figure}[h!]
  ...
  \caption{Panel klienta - analiza ryzyka rezygnacji (Churn Risk Analysis) z poziomem ryzyka i analizą zachowań zakupowych.}
...
\subsection*{4.8 Panel administracyjny - widoki probabilistyczne}
\addcontentsline{toc}{subsection}{4.8 Panel administracyjny - widoki probabilistyczne}
...
\begin{figure}[h!]
  ...
  \caption{Panel administracyjny - analiza łańcucha Markowa z wykresem prognozy sprzedaży (Sales Forecast) i szczegółową tabelą predykcji.}
...
\begin{figure}[h!]
  ...
  \caption{Panel administracyjny - analiza Bayesowska z wizualizacją preferencji kategorii i wskaźnikami wydajności.}
...
Schemat bazy danych PostgreSQL został wygenerowany przez Django ORM. Baza składa się z tabel podzielonych na moduły funkcjonalne.

\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{images/appErd.png}
  \caption{ERD: Główne tabele aplikacji e-commerce (użytkownicy, produkty, zamówienia, opinie, kategorie).}
  \label{fig:app_erd}
\end{figure}
...
Relacje: FK do \texttt{db\_sale}, ManyToMany do \texttt{db\_category}, ManyToMany do \texttt{db\_tag}
...
Kolumny: \texttt{id}, \texttt{username}, \texttt{email}, \texttt{password}, \texttt{role} (VARCHAR 10: 'admin'/'client'), \texttt{is\_staff}, \texttt{is\_active}

\item \textbf{db\_order} -- zamówienia\\
Kolumny: \texttt{id}, \texttt{user\_id} (FK), \texttt{date\_order} (DATETIME), \texttt{status} (VARCHAR 20)

\item \textbf{db\_order\_product} -- produkty w zamówieniach\\
Kolumny: \texttt{id}, \texttt{order\_id} (FK), \texttt{product\_id} (FK), \texttt{quantity} (PositiveIntegerField)\\
Wykorzystywane do budowy sekwencji zakupowych dla łańcucha Markowa
\end{itemize}

\textbf{Moduł Rekomendacji (3 metody)}

\begin{itemize}
\item \textbf{method\_product\_similarity} -- podobieństwa Content-Based Filtering\\
Kolumny: \texttt{id}, \texttt{product1\_id} (FK), \texttt{product2\_id} (FK), \texttt{similarity\_type} (VARCHAR 20: 'content\_based'), \texttt{similarity\_score} (DECIMAL 5,3), \texttt{updated\_at}\\
Constraint: UNIQUE(product1\_id, product2\_id, similarity\_type)\\
Indeks na (product\_1, similarity\_type) dla szybkiego wyszukiwania

\item \textbf{method\_fuzzy\_user\_profile} -- rozmyte profile użytkowników\\
Kolumny: \texttt{id}, \texttt{user\_id} (FK), \texttt{price\_sensitivity} (DECIMAL 5,3), \texttt{category\_preferences} (JSONField), \texttt{quality\_preference} (DECIMAL 5,3), \texttt{updated\_at}

\item \textbf{method\_markov\_transitions} -- macierz przejść łańcucha Markowa\\
Kolumny: \texttt{id}, \texttt{from\_category} (VARCHAR 100), \texttt{to\_category} (VARCHAR 100), \texttt{transition\_probability} (DECIMAL 5,4), \texttt{transition\_count} (INTEGER)\\
Constraint: UNIQUE(from\_category, to\_category)

\item \textbf{method\_user\_purchase\_pattern} -- wzorce zakupowe użytkownika\\
Kolumny: \texttt{id}, \texttt{user\_id} (FK), \texttt{category\_id} (FK), \texttt{purchase\_frequency} (DECIMAL 5,2), \texttt{average\_order\_value} (DECIMAL 10,2), \texttt{last\_computed} (DATETIME)
\end{itemize}

\textbf{Moduł Probabilistyki i Ryzyka}

\begin{itemize}
\item \textbf{method\_purchase\_probability} -- prawdopodobieństwo zakupu\\
Kolumny: \texttt{id}, \texttt{user\_id} (FK), \texttt{product\_id} (FK), \texttt{probability} (DECIMAL 5,3), \texttt{confidence\_level} (DECIMAL 5,3)

\item \textbf{method\_sales\_forecast} -- prognozy sprzedaży\\
Kolumny: \texttt{id}, \texttt{product\_id} (FK), \texttt{forecast\_date} (DATE), \texttt{predicted\_quantity} (INTEGER), \texttt{confidence\_interval\_lower}, \texttt{confidence\_interval\_upper}

\item \textbf{method\_risk\_assessment} -- oceny ryzyka (churn)\\
Kolumny: \texttt{id}, \texttt{risk\_type} (VARCHAR 50), \texttt{entity\_type} (VARCHAR 20), \texttt{entity\_id}, \texttt{risk\_score} (DECIMAL 5,3), \texttt{mitigation\_suggestion} (TEXT NULL)
\end{itemize}

\textbf{RecommendationSettings} -- singleton przechowujący aktywny algorytm rekomendacji wybrany przez administratora (content\_based, fuzzy\_logic, probabilistic).

\subsection*{5.6 Wdrożenie aplikacji — Docker}
\addcontentsline{toc}{subsection}{5.6 Wdrożenie aplikacji — Docker}
