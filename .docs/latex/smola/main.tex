...
Głównym celem pracy jest zaprojektowanie i implementacja systemu rekomendacji produktów dla platformy e-commerce, który łączy trzy zaawansowane metody uczenia maszynowego. System został zbudowany od podstaw, bez wykorzystania gotowych bibliotek uczenia maszynowego, co zapewniło pełną kontrolę nad algorytmami i możliwość ich dostosowania do specyficznych wymagań biznesowych. System został zrealizowany we współpracy dwuosobowej, gdzie każdy z autorów zaimplementował trzy komplementarne metody rekomendacyjne.
...
System rekomendacji produktów został oparty na trzech zaawansowanych metodach uczenia maszynowego, które łączą różne podejścia do problemu personalizacji: filtrację opartą na treści, logikę rozmytą oraz modele probabilistyczne. Metody te zostały wybrane ze względu na komplementarność mechanizmów działania oraz zdolność do rozwiązywania różnych aspektów problemu rekomendacji. System został rozbudowany we współpracy o dodatkowe trzy metody (Collaborative Filtering, analiza sentymentu, reguły asocjacyjne), tworząc kompleksowe rozwiązanie dla platform e-commerce.
...
\subsection*{3.1 Architektura wysokopoziomowa systemu}
\addcontentsline{toc}{subsection}{3.1 Architektura wysokopoziomowa systemu}

System rekomendacji produktów został zaprojektowany jako modułowa aplikacja webowa oparta na architekturze trójwarstwowej. Główne komponenty architektury to:

\textbf{Warstwa prezentacji (Frontend)} — Single Page Application zbudowana w React 18, odpowiedzialna za interfejs użytkownika, wyświetlanie rekomendacji oraz interakcję z API.

\textbf{Warstwa logiki biznesowej (Backend)} — Serwer aplikacyjny Django 5.1.4 z Django REST Framework, implementujący sześć niezależnych silników rekomendacyjnych (trzy w ramach niniejszej pracy: CBF, Fuzzy Logic, modele probabilistyczne; trzy w ramach pracy współautora: Collaborative Filtering, analiza sentymentu, reguły asocjacyjne) oraz udostępniający REST API.

\textbf{Warstwa danych (Database)} — Relacyjna baza danych PostgreSQL 14 przechowująca dane produktów, użytkowników, zamówień oraz prekompilowane wyniki wszystkich sześciu algorytmów (macierz podobieństw CBF i CF, macierz przejść Markowa, profile rozmyte użytkowników, wyniki analizy sentymentu, reguły asocjacyjne).

System został zaprojektowany zgodnie z zasadą separacji odpowiedzialności (Separation of Concerns), gdzie każdy moduł ma jasno zdefiniowaną rolę i komunikuje się z innymi modułami przez dobrze zdefiniowane interfejsy. Architektura została zrealizowana we współpracy dwuosobowej, zapewniając spójną infrastrukturę dla wszystkich metod rekomendacyjnych.

\subsection*{3.2 Projekt modułów rekomendacyjnych}
\addcontentsline{toc}{subsection}{3.2 Projekt modułów rekomendacyjnych}

System składa się z sześciu niezależnych modułów rekomendacyjnych, które mogą działać osobno lub być łączone w system hybrydowy. Niniejsza praca obejmuje szczegółowy opis trzech modułów zaimplementowanych w jej ramach:

\textbf{Moduł Content-Based Filtering}

Zadania modułu:
\begin{itemize}
    \item Ekstrakcja cech produktów (kategorie, tagi, przedział cenowy, słowa kluczowe z opisów)
    \item Reprezentacja produktów jako wektorów w przestrzeni cech
    \item Obliczanie podobieństwa kosinusowego między produktami
    \item Generowanie macierzy podobieństw i jej persystencja w bazie danych
    \item Zwracanie top-N produktów podobnych do danego produktu
\end{itemize}

Główne klasy:
\begin{itemize}
    \item \texttt{CustomContentBasedFilter} — główna klasa silnika CBF
    \item \texttt{ProductSimilarity} (model Django) — przechowuje macierz podobieństw
\end{itemize}

\textbf{Moduł Fuzzy Logic}

Zadania modułu:
\begin{itemize}
    \item Definicja funkcji przynależności dla zmiennych wejściowych (cena, jakość, popularność)
    \item Budowa rozmytych profili użytkowników na podstawie historii zakupów
    \item Ewaluacja reguł rozmytych IF-THEN (6 reguł)
    \item Fuzzyfikacja wartości wejściowych produktu
    \item Agregacja wyników reguł i defuzzyfikacja do wartości liczbowej (fuzzy\_score)
\end{itemize}

Główne klasy:
\begin{itemize}
    \item \texttt{FuzzyMembershipFunctions} — definicje funkcji przynależności
    \item \texttt{FuzzyUserProfile} — budowa profilu rozmytego użytkownika
    \item \texttt{SimpleFuzzyInference} — silnik wnioskowania Mamdani
\end{itemize}

\textbf{Moduł Probabilistic Models}

Zadania modułu:
\begin{itemize}
    \item Trening łańcucha Markowa na sekwencjach kategorii zakupowych
    \item Trening klasyfikatora Naive Bayes na cechach użytkowników
    \item Predykcja następnych kategorii zakupów (Markov)
    \item Predykcja prawdopodobieństwa zakupu i ryzyka churnu (Naive Bayes)
    \item Agregacja wyników obu modeli w spójne rekomendacje
\end{itemize}

Główne klasy:
\begin{itemize}
    \item \texttt{CustomMarkovChain} — implementacja łańcucha Markowa
    \item \texttt{CustomNaiveBayes} — implementacja Naive Bayes z wygładzaniem Laplace'a
    \item \texttt{ProbabilisticRecommendationEngine} — integracja obu modeli
\end{itemize}

System został rozbudowany o trzy dodatkowe moduły rekomendacyjne zaimplementowane przez współautora: moduł Collaborative Filtering wykorzystujący metrykę Adjusted Cosine Similarity, moduł analizy sentymentu oparty na podejściu słownikowym oraz moduł reguł asocjacyjnych wykorzystujący algorytm Apriori. Wszystkie sześć modułów działa w ramach wspólnej architektury backendowej i może być dynamicznie aktywowane przez administratora systemu.

\subsection*{3.3 Diagram klas — modele danych}
\addcontentsline{toc}{subsection}{3.3 Diagram klas — modele danych}

System wykorzystuje następujące główne modele danych (Django ORM):

\textbf{Moduł użytkowników i uwierzytelniania:}
\begin{itemize}
    \item \texttt{User} (extends AbstractUser) — użytkownicy systemu z polem role (admin/client)
    \item \texttt{CartItem} — pozycje w koszyku zakupowym
\end{itemize}

\textbf{Moduł katalogu produktów:}
\begin{itemize}
    \item \texttt{Product} — produkty z atrybutami: name, price, old\_price, description
    \item \texttt{Category} — kategorie produktów (48 kategorii)
    \item \texttt{Tag} — tagi produktowe (np. "Gaming", "Budget", "Premium")
    \item \texttt{Sale} — promocje i wyprzedaże
    \item \texttt{Specification} — specyfikacje techniczne produktów (para klucz-wartość)
\end{itemize}

\textbf{Moduł zamówień:}
\begin{itemize}
    \item \texttt{Order} — zamówienia użytkowników
    \item \texttt{OrderProduct} — pozycje w zamówieniach (tabela pośrednicząca M2M)
\end{itemize}

\textbf{Moduł rekomendacji:}
\begin{itemize}
    \item \texttt{ProductSimilarity} — macierz podobieństw dla CBF
    \item \texttt{FuzzyUserProfile} — profile rozmyte użytkowników
    \item \texttt{MarkovTransitions} — macierz przejść łańcucha Markowa
    \item \texttt{UserPurchasePattern} — wzorce zakupowe użytkowników
    \item \texttt{PurchaseProbability} — prawdopodobieństwa zakupu (Naive Bayes)
    \item \texttt{RecommendationSettings} — singleton z konfiguracją aktywnego algorytmu
\end{itemize}

\textbf{Relacje między modelami:}
\begin{itemize}
    \item Product ↔ Category: Many-to-Many
    \item Product ↔ Tag: Many-to-Many
    \item Product → Sale: Foreign Key (nullable)
    \item User → Order: One-to-Many
    \item Order ↔ Product: Many-to-Many (przez OrderProduct)
    \item ProductSimilarity → Product: dwa Foreign Keys (product1, product2)
\end{itemize}

\subsection*{3.4 Diagram sekwencji — generowanie rekomendacji CBF}
\addcontentsline{toc}{subsection}{3.4 Diagram sekwencji — generowanie rekomendacji CBF}

Proces generowania rekomendacji Content-Based Filtering przebiega zgodnie z następującym scenariuszem:

\begin{enumerate}
    \item Użytkownik przegląda stronę szczegółową produktu w interfejsie React
    \item Komponent ProductPage wysyła żądanie HTTP GET do endpointu \texttt{/api/recommendations/content-based/?product\_id=\{id\}}
    \item Backend (ContentBasedAPI view) odbiera żądanie
    \item System sprawdza cache — jeśli podobieństwa dla produktu są zakeszowane, zwraca je natychmiast
    \item W przypadku braku w cache:
    \begin{itemize}
        \item Wykonuje zapytanie do modelu ProductSimilarity z filtrem na product1\_id
        \item Sortuje wyniki według similarity\_score (malejąco)
        \item Pobiera top 10 najbardziej podobnych produktów
    \end{itemize}
    \item Backend serializuje dane (ProductSerializer) i zwraca odpowiedź JSON 200 OK
    \item Frontend odbiera listę rekomendacji i renderuje sekcję "Podobne produkty"
\end{enumerate}

\subsection*{3.5 Diagram sekwencji — generowanie rekomendacji Fuzzy Logic}
\addcontentsline{toc}{subsection}{3.5 Diagram sekwencji — generowanie rekomendacji Fuzzy Logic}

Proces generowania rekomendacji z wykorzystaniem logiki rozmytej:

\begin{enumerate}
    \item Użytkownik zalogowany otwiera panel klienta (ClientPanel)
    \item Frontend wysyła żądanie GET do \texttt{/api/fuzzy-recommendations/?user\_id=\{id\}}
    \item Backend pobiera profil rozmyty użytkownika z modelu FuzzyUserProfile
    \item Jeśli profil nie istnieje:
    \begin{itemize}
        \item System analizuje historię zamówień użytkownika
        \item Oblicza wrażliwość cenową, preferencje kategorialne, preferencję jakości
        \item Zapisuje nowy profil rozmyty w bazie danych
    \end{itemize}
    \item Dla każdego produktu w katalogu:
    \begin{itemize}
        \item Fuzzyfikacja wartości (cena, rating, view\_count)
        \item Ewaluacja 6 reguł rozmytych z użyciem T-norm (minimum)
        \item Agregacja wyników reguł
        \item Obliczenie fuzzy\_score metodą średniej ważonej
    \end{itemize}
    \item Sortowanie produktów według fuzzy\_score
    \item Zwrócenie top 10 produktów z najwyższym wynikiem
    \item Frontend renderuje rekomendacje w zakładce "Fuzzy Recommendations"
\end{enumerate}

\subsection*{3.6 Diagram sekwencji — generowanie rekomendacji Probabilistic}
\addcontentsline{toc}{subsection}{3.6 Diagram sekwencji — generowanie rekomendacji Probabilistic}

Proces generowania rekomendacji probabilistycznych (Markov + Naive Bayes):

\begin{enumerate}
    \item Użytkownik otwiera zakładkę "Smart Recommendations" w panelu klienta
    \item Frontend wysyła żądanie GET do \texttt{/api/markov-recommendations/?user\_id=\{id\}}
    \item Backend sprawdza czy modele są wytrenowane:
    \begin{itemize}
        \item Jeśli nie — uruchamia trening łańcucha Markowa na sekwencjach zakupowych wszystkich użytkowników
        \item Jeśli nie — trenuje Naive Bayes na cechach użytkowników (zakup/churn)
    \end{itemize}
    \item System pobiera ostatnie zamówienie użytkownika i ekstrahuje kategorię ostatnio kupionego produktu
    \item Łańcuch Markowa przewiduje top 5 najbardziej prawdopodobnych następnych kategorii
    \item Naive Bayes oblicza prawdopodobieństwo zakupu dla użytkownika
    \item System agreguje wyniki: 60\% waga Markova + 40\% waga Naive Bayes
    \item Dla przewidywanych kategorii pobiera produkty z bazy
    \item Zwraca top 10 produktów z najwyższym zagregowanym score
    \item Frontend wyświetla rekomendacje wraz z metrykami (purchase probability, expected days to next order)
\end{enumerate}

\subsection*{3.7 Diagram przypadków użycia}
\addcontentsline{toc}{subsection}{3.7 Diagram przypadków użycia}

System obsługuje trzech głównych aktorów z różnymi poziomami uprawnień oraz zestaw przypadków użycia związanych z funkcjonalnościami rekomendacyjnymi oraz e-commerce. Relacje dziedziczenia między aktorami (Gość $\rightarrow$ Klient $\rightarrow$ Administrator) odzwierciedlają hierarchię uprawnień — każdy następny poziom dziedziczy wszystkie funkcjonalności poprzedniego i dodaje nowe, specyficzne dla swojej roli.

\textbf{Aktorzy systemu:}

\textbf{Gość (użytkownik niezalogowany):}
\begin{itemize}
...
\textbf{Klient (użytkownik zalogowany):}
\begin{itemize}
    ...
    \item Przeglądanie spersonalizowanych rekomendacji produktów (CBF, Fuzzy Logic, Probabilistic, Collaborative Filtering),
    \item Rekomendacje metody reguł asocjacyjnych w koszyku (produkty często kupowane razem),
    \item Wyszukiwanie produktów z uwzględnieniem analizy sentymentu opinii.
\end{itemize}
...
\begin{itemize}
    ...
    \item Trening modeli probabilistycznych i analizy sentymentu,
    \item Zmiana aktywnych algorytmów rekomendacji w systemie.
\end{itemize}

\subsection*{3.8 Projekt API — endpointy REST}
\addcontentsline{toc}{subsection}{3.8 Projekt API — endpointy REST}

System udostępnia REST API zbudowane w Django REST Framework. Zaprojektowano następujące grupy endpointów:

\textbf{API Content-Based Filtering:}
\begin{itemize}
    \item \texttt{GET /api/recommendations/content-based/?product\_id=\{id\}} — rekomendacje dla produktu
    \item \texttt{GET /api/content-based-debug/} — panel debugowania CBF (admin)
    \item \texttt{POST /api/generate-similarities/} — generowanie macierzy podobieństw (admin)
\end{itemize}

\textbf{API Fuzzy Logic:}
\begin{itemize}
    \item \texttt{GET /api/fuzzy-recommendations/?user\_id=\{id\}} — rekomendacje rozmyte dla użytkownika
    \item \texttt{GET /api/fuzzy-debug/?user\_id=\{id\}} — panel debugowania Fuzzy (admin)
    \item \texttt{GET /api/fuzzy-debug/?product\_id=\{id\}} — analiza fuzzyfikacji produktu (admin)
\end{itemize}

\textbf{API Probabilistic Models:}
\begin{itemize}
    \item \texttt{GET /api/markov-recommendations/?user\_id=\{id\}} — rekomendacje Markov + NB
    \item \texttt{GET /api/bayesian-insights/?user\_id=\{id\}} — analiza behawioralna użytkownika
    \item \texttt{GET /api/probabilistic-debug/} — panel debugowania Markov + NB (admin)
\end{itemize}

\textbf{API Collaborative Filtering (współautor):}
\begin{itemize}
    \item \texttt{GET /api/generate-user-recommendations/} — rekomendacje oparte na wzorcach użytkowników
    \item \texttt{GET /api/collaborative-debug/} — panel debugowania CF (admin)
\end{itemize}

\textbf{API Sentiment Analysis (współautor):}
\begin{itemize}
    \item \texttt{GET /api/sentiment-search/} — wyszukiwanie z uwzględnieniem sentymentu opinii
    \item \texttt{GET /api/sentiment-debug/} — panel debugowania analizy sentymentu (admin)
\end{itemize}

\textbf{API Association Rules (współautor):}
\begin{itemize}
    \item \texttt{GET /api/frequently-bought-together/} — produkty często kupowane razem
    \item \texttt{GET /api/association-debug/} — panel debugowania reguł asocjacyjnych (admin)
\end{itemize}

\textbf{API konfiguracji:}
\begin{itemize}
    \item \texttt{GET /api/recommendation-settings/} — pobranie aktywnego algorytmu
    \item \texttt{PATCH /api/recommendation-settings/} — zmiana aktywnego algorytmu (admin)
\end{itemize}

\textbf{Format odpowiedzi API}

Wszystkie endpointy zwracają dane w formacie JSON. Przykładowa odpowiedź dla rekomendacji CBF:
...
\subsection*{3.9 Projekt bazy danych — model ERD}
\addcontentsline{toc}{subsection}{3.9 Projekt bazy danych — model ERD}

Baza danych została zaprojektowana zgodnie z trzecią postacią normalną (3NF) z wykorzystaniem Django ORM. Struktura składa się z pięciu modułów funkcjonalnych:

\textbf{Moduł produktów i kategorii:}
\begin{itemize}
    \item Tabela \texttt{db\_product} — 500 produktów, klucz główny (id), atrybuty: name, price, old\_price, description
    \item Tabela \texttt{db\_category} — 48 kategorii, constraint UNIQUE na nazwie
    \item Tabela pośrednicząca \texttt{product\_categories} — relacja M2M
    \item Tabela \texttt{db\_specification} — specyfikacje jako pary (parameter\_name, specification)
\end{itemize}

\textbf{Moduł użytkowników i zamówień:}
\begin{itemize}
    \item Tabela \texttt{db\_user} — extends Django AbstractUser, dodatkowe pole role
    \item Tabela \texttt{db\_order} — zamówienia z datą i statusem
    \item Tabela \texttt{db\_order\_product} — pozycje zamówień z ilością
\end{itemize}

\textbf{Moduł rekomendacji CBF:}
\begin{itemize}
    \item Tabela \texttt{method\_product\_similarity} — macierz podobieństw
    \item Indeks na (product1\_id, similarity\_type) dla wydajności
    \item Constraint UNIQUE(product1\_id, product2\_id, similarity\_type)
\end{itemize}

\textbf{Moduł rekomendacji Fuzzy:}
\begin{itemize}
    \item Tabela \texttt{method\_fuzzy\_user\_profile} — profile użytkowników
    \item Pole category\_preferences typu JSONField (elastyczna struktura)
    \item Pola: price\_sensitivity, quality\_preference, updated\_at
\end{itemize}

\textbf{Moduł rekomendacji Probabilistic:}
\begin{itemize}
    \item Tabela \texttt{method\_markov\_transitions} — macierz przejść Markova
    \item Constraint UNIQUE(from\_category, to\_category)
    \item Tabela \texttt{method\_purchase\_probability} — predykcje Naive Bayes
\end{itemize}

Diagramy ERD znajdują się w rozdziale 4.6 (Baza danych — modele).

\subsection*{3.10 Podsumowanie projektu}
\addcontentsline{toc}{subsection}{3.10 Podsumowanie projektu}

Zaprojektowany system rekomendacyjny charakteryzuje się:

\textbf{Modułowością} — każdy algorytm jest niezależnym modułem, który można aktywować, testować i rozwijać osobno.

\textbf{Skalowalnością} — wykorzystanie indeksów bazodanowych, cache'owania wyników oraz bulk operations zapewnia wydajność dla katalogów do 10000 produktów.

\textbf{Rozszerzalnością} — architektura umożliwia łatwe dodawanie nowych metod rekomendacyjnych poprzez implementację wspólnego interfejsu.

\textbf{Interpretowal}nością — zwłaszcza moduł Fuzzy Logic oferuje pełne wyjaśnienie rekomendacji przez aktywacje reguł.

Projekt został zrealizowany zgodnie z najlepszymi praktykami inżynierii oprogramowania: separacja warstw, wykorzystanie wzorców projektowych (singleton dla RecommendationSettings), RESTful API design oraz normalizacja bazy danych.

\newpage

\section*{Rozdzia\l{} 4}
\addcontentsline{toc}{section}{Rozdział 4: Stos technologiczny i architektura techniczna}
\section*{Stos technologiczny i architektura techniczna}

Niniejszy rozdział jest wspólny z pracą współautora i opisuje szczegóły techniczne całego systemu e-commerce z modułem rekomendacyjnym.

\subsection*{4.1 Architektura systemu i stos technologiczny}
\addcontentsline{toc}{subsection}{4.1 Architektura systemu i stos technologiczny}
...
\textbf{Backend}: Django 5.1.4 (Python 3.11) wraz z Django REST Framework 3.15.2. Django zapewnia solidną architekturę MVC, system ORM (Object-Relational Mapping - mapowanie obiektowo-relacyjne) dla abstrakcji bazy danych oraz mechanizmy bezpieczeństwa (CSRF, XSS, SQL injection prevention). Django REST Framework rozszerza Django o funkcjonalności API RESTful (serializery, ViewSets, uwierzytelnianie, paginacja).

\textbf{Frontend}: React 18 z bibliotekami wspierającymi (Axios, Framer Motion, React Router) tworzy Single Page Application (SPA). React Hooks (useState, useEffect, useContext) zarządzają stanem aplikacji. Context API zapewnia globalny stan (AuthContext, CartContext) bez zewnętrznych bibliotek.

\textbf{Baza danych}: PostgreSQL 14 przechowuje wszystkie dane aplikacji (25 tabel w 4 modułach funkcjonalnych). Wybór PostgreSQL był podyktowany zaawansowanymi funkcjami (indeksy B-tree/GIN/BRIN, full-text search, typ JSONB, transakcje ACID, klucze obce).

\textbf{Biblioteki Machine Learning}: NumPy 1.24 (operacje macierzowe dla wektorów cech i macierzy podobieństwa w CBF i modelu probabilistycznym), scikit-learn (funkcja cosine\_similarity() dla Collaborative Filtering zaimplementowanego przez współautora).

\subsection*{4.2 Backend — Django REST Framework}
\addcontentsline{toc}{subsection}{4.2 Backend — Django REST Framework}

Architektura backendu opiera się na wzorcu Model-View-Serializer (MVS). Django zapewnia komponenty ORM, Signals oraz Middleware:
...
\textbf{Django REST Framework 3.15.2} rozszerza Django o funkcjonalności API RESTful:
...
\textbf{Struktura plików backendu}

Każdy komponent systemu posiada dedykowane pliki. Niniejsza praca obejmuje następujące moduły:

\begin{itemize}
    \item \textbf{models.py} - definicje tabel (Product, Order, ProductSimilarity, FuzzyUserProfile, MarkovTransitions, PurchaseProbability, RecommendationSettings),
    ...
    \item \textbf{fuzzy\_logic\_engine.py} - implementacja systemu Fuzzy Logic (Mamdani),
    \item \textbf{recommendation\_views.py} - endpointy CBF: \texttt{/api/content-based-debug/}, \texttt{/api/generate-similarities/},
    \item \textbf{fuzzy\_views.py} - endpointy Fuzzy: \texttt{/api/fuzzy-recommendations/}, \texttt{/api/fuzzy-debug/},
    \item \textbf{probabilistic\_views.py} - endpointy Markov i Bayes: \texttt{/api/markov-recommendations/}, \texttt{/api/bayesian-insights/},
    \item \textbf{signals.py} - automatyczna aktualizacja rekomendacji przy zmianach danych.
\end{itemize}

System został rozbudowany przez współautora o dodatkowe pliki:

\begin{itemize}
    \item \textbf{collaborative\_views.py} - endpointy \texttt{/api/generate-user-recommendations/}, \texttt{/api/recommendation-algorithm-status/},
    \item \textbf{sentiment\_views.py} - endpointy \texttt{/api/sentiment-search/}, \texttt{/api/sentiment-analysis-debug/},
    \item \textbf{association\_views.py} - endpointy \texttt{/api/frequently-bought-together/}, \texttt{/api/update-association-rules/}.
\end{itemize}

\subsection*{4.3 Frontend — React 18}
\addcontentsline{toc}{subsection}{4.3 Frontend — React 18}
...
\subsection*{4.4 Diagram przypadków użycia}
\addcontentsline{toc}{subsection}{4.4 Diagram przypadków użycia}
...
\subsection*{4.5 Baza danych — modele ERD}
\addcontentsline{toc}{subsection}{4.5 Baza danych — modele ERD}

Schemat bazy danych PostgreSQL został zaprojektowany zgodnie z modelem Django ORM. Struktura bazy danych jest podzielona na moduły funkcjonalne realizujące logikę systemu rekomendacji e-commerce.

\textbf{1. Moduł produktów i użytkowników (12 tabel):}

\begin{itemize}
\item \texttt{db\_product} -- dane produktów (ID, nazwa, cena, old\_price, opis, rating, view\_count),
\item \texttt{db\_category} -- kategorie produktów z hierarchią (48 kategorii z constraint UNIQUE na nazwie),
\item \texttt{db\_product\_category} -- relacja Many-to-Many produktów i kategorii,
\item \texttt{db\_photo\_product} -- ścieżki do zdjęć produktów (wielokrotne zdjęcia per produkt),
\item \texttt{db\_specification} -- szczegółowe parametry techniczne produktów (klucz-wartość),
\item \texttt{db\_tag} -- tagi do filtrowania produktów (np. "Gaming", "Premium", "Budget"),
\item \texttt{db\_sale} -- promocje i rabaty (powiązanie z Product),
\item \texttt{db\_user} -- konta użytkowników (extends Django AbstractUser, dodatkowe pole role),
\item \texttt{db\_order} -- zamówienia z timestampami i statusami,
\item \texttt{db\_order\_product} -- produkty w zamówieniach (ilość, cena jednostkowa),
...
\begin{itemize}
\item \texttt{db\_opinion} -- opinie użytkowników (treść, rating 1-5, timestamps),
\item \texttt{method\_sentiment\_analysis} -- wyniki analizy sentymentu dla opinii (współautor),
\item \texttt{method\_product\_sentiment\_summary} -- zagregowany sentyment produktu (współautor).
\end{itemize}

\textbf{3. Moduł metod rekomendacji (5 tabel):}

\begin{itemize}
\item \texttt{method\_product\_similarity} -- macierz podobieństw (CBF i CF), constraint UNIQUE(product1\_id, product2\_id, similarity\_type), indeks na (product1\_id, similarity\_type),
\item \texttt{method\_fuzzy\_user\_profile} -- profile rozmyte użytkowników (price\_sensitivity, quality\_preference, category\_preferences jako JSONField),
\item \texttt{method\_markov\_transitions} -- macierz przejść Markova (from\_category, to\_category, probability, count), constraint UNIQUE(from\_category, to\_category),
\item \texttt{method\_purchase\_probability} -- predykcje Naive Bayes (user\_id, purchase\_probability, expected\_days\_to\_next\_order),
\item \texttt{method\_user\_product\_recommendation} -- cache rekomendacji użytkownika (współautor),
\item \texttt{method\_productassociation} -- reguły asocjacyjne Apriori (współautor),
\item \texttt{method\_user\_interactions} -- historia interakcji użytkowników,
\item \texttt{method\_recommendation\_settings} -- konfiguracja algorytmów (wzorzec Singleton).
\end{itemize}
...
\item \texttt{method\_user\_purchase\_pattern} -- wzorce zakupowe użytkowników (typ JSONB),
...
Wszystkie migracje Django ORM zostały wygenerowane automatycznie na podstawie modeli Python. Wypełnianie danymi początkowymi (seeding): 500 produktów, 20 użytkowników, 200 zamówień, $\sim$600 rekordów OrderProduct, $\sim$1750 opinii.
...
Diagram \ref{fig:erd1} przedstawia rdzeń aplikacji e-commerce. Kluczowe relacje: db\_user $\rightarrow$ db\_order (1:N), db\_order $\rightarrow$ db\_order\_product (1:N), db\_product $\leftrightarrow$ db\_category (N:M), db\_product $\rightarrow$ db\_opinion (1:N).
...
Diagram \ref{fig:erd2} pokazuje tabele algorytmów ML: method\_product\_similarity (CBF i CF), method\_fuzzy\_user\_profile (Fuzzy Logic), method\_markov\_transitions i method\_purchase\_probability (modele probabilistyczne), method\_productassociation (Apriori - współautor), method\_sentiment\_analysis i method\_product\_sentiment\_summary (analiza sentymentu - współautor), method\_user\_product\_recommendation (cache CF - współautor).

\newpage

\section*{Rozdzia\l{} 5}
\addcontentsline{toc}{section}{Rozdział 5: Implementacja Content-Based Filtering}
\section*{Implementacja Content-Based Filtering}
