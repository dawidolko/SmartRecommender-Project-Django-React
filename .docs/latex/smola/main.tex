...
Rysunek \ref{fig:erd1_v2} przedstawia rdzeń aplikacji e-commerce. Diagram ilustruje podstawowe tabele systemu oraz ich relacje. Tabela \texttt{db\_user} zawiera dane użytkowników systemu i jest powiązana relacją jeden-do-wielu z tabelą \texttt{db\_order}, reprezentującą zamówienia. Pojedyncze zamówienie agreguje wiele produktów poprzez tabelę pośrednią \texttt{db\_order\_product}, która przechowuje szczegóły pozycji zamówienia (ilość, cena jednostkowa). Tabela \texttt{db\_product} łączy się z \texttt{db\_category} relacją wiele-do-wielu, umożliwiając przynależność produktu do wielu kategorii jednocześnie. Dodatkowo produkty są powiązane z opiniami użytkowników w relacji jeden-do-wielu z tabelą \texttt{db\_opinion}.
...
\begin{itemize}
    ...
    \item Kategorie: low (<100), medium (100-500), high (500-1500), premium (≥1500)
\end{itemize}

Wagi zostały dobrane empirycznie poprzez testowanie różnych konfiguracji na rzeczywistym katalogu produktów. Zmiana parametrów wymaga modyfikacji kodu źródłowego i restartu serwera Django, po czym wymagane jest ponowne wygenerowanie macierzy podobieństw poprzez wywołanie endpointa \texttt{POST /api/generate-similarities/}.
...
\begin{itemize}
    ...
    \item Brak wygładzania Laplace'a -- MLE bez wygładzania
\end{itemize}
...
Niniejszy podrozdział przedstawia sposób prezentacji rekomendacji w interfejsie użytkownika końcowego oraz w panelach administracyjnych. System SmartRecommender oferuje zaimplementowane trzy metody rekomendacyjne -- Content-Based Filtering, Fuzzy Logic oraz modele probabilistyczne -- które są dostępne dla użytkowników poprzez intuicyjny interfejs webowy. Użytkownik końcowy otrzymuje spersonalizowane sugestie produktowe bez konieczności rozumienia mechanizmów działania algorytmów, natomiast administrator dysponuje zaawansowanymi narzędziami debugowania i monitorowania jakości rekomendacji.
...
\begin{itemize}
    ...
    \item Dekompozycja podobieństwa: ile procent z kategorii, ile z tagów, ile z ceny, ile ze słów kluczowych
\end{itemize}
...
Panel wyświetla następujące sekcje (Rysunek \ref{fig:fuzzy_debug1}):

\begin{itemize}
    \item \textbf{Algorithm Information}:
    \begin{itemize}
        \item Name: Fuzzy Logic Inference System (Mamdani-Style)
        \item Description: System rekomendacji oparty na logice rozmytej z uproszczoną defuzzyfikacją
    \end{itemize}
    \item \textbf{User Profile} -- profil rozmyty użytkownika gościa:
    \begin{itemize}
        \item User: Guest/Default
        \item Profile Type: session-based
        \item Price Sensitivity: 0.6-Medium
        \item Tracked Categories: 3
        \item Category Interests: Electronics (0.5), Home (0.4), Gaming (0.3)
    \end{itemize}
\end{itemize}

\noindent Na zrzucie ekranu jako przykładowy produkt wybrano kamerkę A4Tech HD PK-910P do dalszej analizy.
...
Panel przedstawia stopnie przynależności (membership degrees) dla kamerki A4Tech HD PK-910P (Rysunek \ref{fig:fuzzy_debug4}):

\begin{itemize}
    \item \textbf{Fuzzification - Membership Degrees}:
    \begin{itemize}
        \item Price: 238.95 PLN -- Cheap ($\varepsilon=1.0$), Medium ($\varepsilon=0$), Expensive ($\varepsilon=0$), Dominant: CHEAP
        \item Quality: 5.0 -- Low ($\varepsilon=0$), Medium ($\varepsilon=0$), High ($\varepsilon=1.0$), Dominant: HIGH
        \item Popularity: 3 items -- Low ($\varepsilon=0$), Medium ($\varepsilon=1.0$), High ($\varepsilon=0$), Dominant: LOW
    \end{itemize}
    \item \textbf{Category Matching}:
    \begin{itemize}
        \item Max Match: 0.44
        \item Category Details: peripherals/webcams (kategoria "urządzenia peryferyjne/kamery internetowe")
    \end{itemize}
\end{itemize}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\textwidth]{images/fuzzyLogicAdminDebug4.png}
...
Panel przedstawia aktywację reguł IF-THEN oraz proces defuzzyfikacji (Rysunek \ref{fig:fuzzy_debug5}):

\begin{itemize}
    \item \textbf{Fuzzy Inference - Rule Activations} -- aktywacja 6 reguł:
    \begin{itemize}
        \item R1: High Quality Bargain (Okazja wysokiej jakości): 1.0
        \item R2: Popular in Category (Popularny w kategorii): 0
        \item R3: Price Sensitive Match (Dopasowanie wrażliwości cenowej): 0
        \item R4: Category Quality Match (Dopasowanie jakości kategorii): 0.44
        \item R5: Premium Match (Dopasowanie premium): 0
        \item R6: Quality-Price Balance (Balans jakość-cena): 0.8
    \end{itemize}
    \item \textbf{Defuzzification} -- agregacja wyników:
    \begin{itemize}
        \item Method: Weighted Average (średnia ważona)
        \item Final Fuzzy Score: 0.407
        \item Percentage: 40.7\%
        \item Defuzzyfikacja: wzór $\frac{weighted\_sum}{weight\_sum}$
        \item Weighted Sum: 1.874
        \item Weight Sum: 4.6
    \end{itemize}
\end{itemize}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\textwidth]{images/fuzzyLogicAdminDebug5.png}
...
Panel oferuje dwa główne widoki: panele administracyjne (prognozy sprzedaży, analiza popytu) oraz panel debugowania (statystyki modeli Markov Chain i Naive Bayes). Poniżej przedstawiono szczegółowy opis obu widoków.
...
\textbf{Collaborative Filtering (algorytm współautora)} wykorzystuje zapisy transakcyjne użytkowników do konstrukcji macierzy podobieństw produktowych w oparciu o znormalizowane podobieństwo kosinusowe (Adjusted Cosine Similarity) z centralizacją względem średnich użytkowników. Algorytm identyfikuje asocjacje niewidoczne w analizie atrybutowej - przykładowo, częste współ występowanie aparatów fotograficznych i plecaków turystycznych w koszykach zakupowych prowadzi do ich połączenia mimo odmiennej klasyfikacji kategorialnej. Metoda charakteryzuje się podatnością na problem zimnego startu (nowe produkty bez historii transakcyjnej) oraz wymaga znacznego wolumenu danych historycznych dla efektywnego działania. Wyniki zapisywane są w encji \texttt{ProductSimilarity} z oznaczeniem \texttt{type='collaborative'}. Algorytm wykazuje szczególną efektywność w odkrywaniu rekomendacji międzyka tegorialnych (\textit{cross-category recommendations}), zwiększających wartość średniego koszyka.

\textbf{Sentiment Analysis (algorytm współautora)} przeprowadza ekstrakcję tonu emocjonalnego z pięciu źródeł informacji: teksty opinii użytkowników (waga 40\%), opis produktu (25\%), nazwa produktu (15\%), specyfikacje techniczne (12\%) oraz nazwy kategorii (8\%). Implementacja wykorzystuje słownik sentymentu Opinion Lexicon (Hu \& Liu 2004) zawierający listy słów pozytywnych i negatywnych. Algorytm oblicza wynik sentymentu według wzoru: (liczba\_słów\_pozytywnych - liczba\_słów\_negatywnych) / liczba\_wszystkich\_słów, generując wskaźnik w dziedzinie [-1, +1]. Mechanizm agregacji wieloźródłowej syntetyzuje wszystkie sygnały w unifikowany wskaźnik \texttt{sentiment\_score}, reprezentujący zagregowany ton wobec produktu. Algorytm charakteryzuje się uniwersalnością (identyczne wyniki niezależnie od użytkownika) i brakiem mechanizmów personalizacyjnych. Kluczowym ograniczeniem jest podatność na konstrukcje negujące, ironiczne oraz sarkastyczne w materiale tekstowym. Rezultaty cachowane są w encji \texttt{method\_product\_sentiment\_summary} i wykorzystywane do rangowania produktów według odbioru społecznego.
