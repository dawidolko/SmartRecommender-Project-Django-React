...
\begin{itemize}
\item \textbf{scikit-learn} - funkcja cosine\_similarity() dla filtracji kolaboratywnej (Collaborative Filtering)
...
\textbf{1. Kontener backendu (Django 5.1.4)}
\begin{itemize}
...
Szczegółowa implementacja trzech algorytmów rekomendacyjnych wraz z pseudokodami oraz diagramami sekwencji przedstawia praktyczne aspekty realizacji metod filtracji kolaboratywnej, analizy sentymentu oraz reguł asocjacyjnych. Każda metoda została zaimplementowana od podstaw bez użycia gotowych bibliotek rekomendacyjnych.
...
Algorytm CF (Collaborative Filtering) oblicza podobieństwa między produktami na podstawie historii zakupów użytkowników. Proces składa się z trzech głównych etapów: budowy macierzy użytkownik-produkt, normalizacji metodą mean-centering oraz obliczenia podobieństw cosinusowych. Normalizacja polega na odjęciu od każdej wartości zakupowej średniej zakupów danego użytkownika, co eliminuje zniekształcenia wynikające z różnych skal (hurtownik kupujący po 100 sztuk vs klient kupujący po 1 sztuce otrzymują porównywalne wagi).

\textbf{Pseudokod 1a: Budowa i normalizacja macierzy CF}
...
\begin{itemize}
\item Pamięć podręczna (cache) przechowuje macierz podobieństw przez 24 godziny
...
\item Wyniki zapisywane są do pamięci podręcznej (24h) oraz tabeli ProductSimilarity
...
Analiza sentymentu pojedynczego tekstu metodą słownikową (Liu 2012) polega na normalizacji tekstu (konwersja na małe litery, usunięcie znaków specjalnych), tokenizacji oraz zliczaniu słów pozytywnych i negatywnych. Słowniki zawierają ~200 słów każdego typu (np. pozytywne: "excellent", "recommend", "quality"; negatywne: "bad", "poor", "disappointing"). Formuła wyniku: (liczba\_pozytywnych - liczba\_negatywnych) / liczba\_wszystkich\_słów. Wynik ograniczany do [-1, 1] i kategoryzowany: pozytywny (>0.1), negatywny (<-0.1), neutralny (inne).
...
\begin{itemize}
\item Przycinanie bitmapowe (Zaki 2000): znaczące przyspieszenie operacji
...
\begin{itemize}
\item Przycinanie bitmapowe (Zaki 2000) dla optymalizacji obliczeń
...
\begin{itemize}
\item CF: pierwsza generacja wymaga przetworzenia macierzy, kolejne zapytania korzystają z pamięci podręcznej
...
\begin{itemize}
\item \texttt{/api/recommendations/collaborative/} - zwraca produkty filtracji kolaboratywnej
\item \texttt{/api/recommendations/sentiment/} - zwraca produkty według sentymentu
\item \texttt{/api/recommendations/association/} - zwraca reguły asocjacyjne
\item \texttt{/api/admin/recommendation-settings/} - zarządzanie aktywną metodą
\end{itemize}

\textbf{Warstwa danych (PostgreSQL)} - tabele przechowujące prekalkulowane wyniki:
\begin{itemize}
\item \texttt{method\_product\_similarity} - macierz podobieństw filtracji kolaboratywnej
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/mainSectionView3.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/searchSentimentView.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/associationView.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/associationAdmin.png}
