...
\noindent
\textbf{Przeglądanie i wyszukiwanie produktów:}
\begin{itemize}
\item Przeglądanie katalogu produktów,
\item Wyszukiwanie produktów full-text w nazwie i opisie,
\item Filtrowanie według kategorii, zakresu cenowego, sentymentu,
\item Sortowanie według ceny, popularności, sentymentu.
\end{itemize}
...
\item Przeglądanie aktywności użytkowników.
\end{itemize}
...
\item Analiza dekompozycji sentymentu produktów,
...
Proces implementacji składa się z dwóch głównych etapów: (1) znajdowania częstych par produktów z obliczeniem Support według wzoru (3), oraz (2) generowania reguł z obliczeniem Confidence i Lift według wzorów (4) i (5). Kluczową optymalizacją jest przycinanie bitmapowe (Zaki 2000), które reprezentuje transakcje jako liczby całkowite z ustawionymi bitami odpowiadającymi produktom. Sprawdzenie czy transakcja zawiera parę produktów wymaga jednej operacji bitowej AND zamiast iteracji po liście, co znacząco przyspiesza obliczenia. Wczesne przycinanie eliminuje rzadkie produkty przed obliczaniem par - dzięki właściwości antymonotoniczności Apriori (jeśli produkt jest rzadki, wszystkie jego pary też są rzadkie) zmniejsza liczbę kandydatów o około 80-90\%.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{images/mainSectionView1.png}
...
\subsection*{6.6 Charakterystyka funkcjonowania systemu}
...
Rozdział przedstawia analizę porównawczą trzech zaimplementowanych metod rekomendacyjnych pod względem merytorycznym oraz wydajnościowym. Celem jest ocena jakości generowanych rekomendacji, identyfikacja mocnych i słabych stron każdej metody oraz wykazanie ich komplementarności w różnych kontekstach użycia systemu e-commerce.

\subsection*{7.1 Porównanie jakości rekomendacji w różnych kontekstach}

\textbf{Strona główna - eksploracja katalogu}

W kontekście strony głównej najbardziej wartościową metodą jest Collaborative Filtering. Obserwacja działania systemu pokazuje, że użytkownicy z historią minimum 3 zakupów otrzymują średnio 15-20 zróżnicowanych rekomendacji produktowych. Kluczowa obserwacja: około 60\% sugerowanych produktów pochodzi z kategorii różnych od wcześniej kupowanych przez użytkownika. Przykładowo, użytkownik kupujący elektronikę (laptop, słuchawki) otrzymuje rekomendacje artykułów biurowych (biurko, krzesło ergonomiczne), ponieważ inni użytkownicy o podobnych wzorcach zakupowych kupowali te produkty razem. Ta zdolność do eksploracji katalogu i sugerowania użytkownikowi produktów, o których istnieniu mógł nie wiedzieć, stanowi unikalną wartość metody CF. 

Dla porównania, sortowanie według sentymentu zawsze promuje te same uniwersalnie najlepiej ocenione produkty niezależnie od profilu użytkownika, co prowadzi do monotonii - różni użytkownicy widzą identyczne top produkty. Apriori w tym kontekście nie ma zastosowania, ponieważ wymaga już wybranych produktów jako punktu odniesienia.

\textbf{Koszyk zakupowy - cross-selling}

W kontekście koszyka zakupowego algorytm Apriori wykazuje bezkonkurencyjną precyzję. Analiza wygenerowanych reguł pokazuje, że reguły z lift > 2.0 wskazują na produkty funkcjonalnie komplementarne z bardzo wysokim prawdopodobieństwem współzakupu. Konkretny przykład z systemu: dla produktu ,,kamera sportowa GoPro'' system identyfikuje następujące reguły:
\begin{itemize}
\item Karta pamięci 64GB (lift=2.8, confidence=0.72) - 72\% użytkowników kupujących kamerę dokupuje kartę pamięci,
\item Futerał ochronny (lift=2.3, confidence=0.58) - praktyczna ochrona sprzętu,
\item Statyw elastyczny (lift=1.9, confidence=0.45) - akcesoria rozszerzające możliwości użytkowania.
\end{itemize}

Są to produkty, których współzakup ma racjonalne uzasadnienie użytkowe - kamera wymaga karty pamięci do działania, futerał zapewnia ochronę, statyw rozszerza możliwości filmowania. Collaborative Filtering w tym kontekście sugerowałby inne kamery (produkty zastępcze), co jest merytorycznie mniej wartościowe dla użytkownika mającego już kamerę w koszyku. Analiza sentymentu mogłaby wskazać najlepiej ocenione kamery, co również nie rozwiązuje potrzeby cross-sellingu.

\textbf{Wyszukiwarka - filtrowanie według jakości}

Analiza sentymentu stanowi jedyną metodę umożliwiającą uniwersalne sortowanie wszystkich produktów według obiektywnej oceny jakości. Kluczowa obserwacja: produkty o sentymencie >0.5 wykazują konsensus pozytywnych opinii w połączeniu z profesjonalnym opisem i specyfikacjami wskazującymi na wysoką jakość. System agreguje sentyment z pięciu źródeł, co eliminuje problemy jednostronnych lub zmanipulowanych ocen.

Przykład praktyczny: użytkownik wyszukuje ,,mysz bezprzewodowa'' i wybiera sortowanie według sentymentu. Wyniki prezentują produkty o potwierdzonej jakości (Logitech MX Master 3, sentiment=0.68) przed tańszymi alternatywami o gorszych opiniach (generyczne modele, sentiment=0.12), mimo że te ostatnie mogą mieć wyższe wskaźniki sprzedaży. Metoda jest szczególnie wartościowa dla użytkowników, którzy nie mają jasno określonych preferencji produktowych (nie wiedzą jakie marki czy modele wybrać), ale chcą ograniczyć wybór do produktów o potwierdzonej jakości przez innych kupujących.

Collaborative Filtering i Apriori nie oferują uniwersalnego rankingu jakości - CF wymaga historii użytkownika, Apriori wymaga już wybranych produktów.

\subsection*{7.2 Komplementarność i wzajemne kompensowanie ograniczeń}

\textbf{Rozwiązanie problemu zimnego startu}

Metody wzajemnie kompensują fundamentalne ograniczenia. Collaborative Filtering i Apriori całkowicie zawodzą dla nowych produktów bez historii transakcyjnej - algorytmy nie mają danych wejściowych do analizy. W tym momencie analiza sentymentu przejmuje rolę głównej metody rekomendacyjnej. Konkretny scenariusz:

\textit{Dzień 1:} Nowy produkt ,,Słuchawki XYZ Pro'' zostaje dodany do katalogu. Analiza sentymentu natychmiast przetwarza opis (,,wysokiej jakości, profesjonalne, komfortowe''), nazwę (,,Pro'' sugeruje segment premium) i specyfikacje (,,redukcja szumów, czas pracy 30h'') generując wynik sentymentu 0.54 (kategoryzowany jako pozytywny). Produkt jest od razu dostępny w wyszukiwarce z sortowaniem według sentymentu. CF i Apriori: brak rekomendacji (0 transakcji).

\textit{Dzień 7:} Produkt zebrał pierwsze 5 opinii użytkowników (średnia 4.2/5 gwiazdek). Analiza sentymentu aktualizuje wynik do 0.61 uwzględniając opinie (waga 40\%). CF i Apriori: nadal brak rekomendacji (zbyt mało transakcji).

\textit{Miesiąc 1:} Produkt ma już 50 transakcji. Collaborative Filtering identyfikuje podobieństwa do innych słuchawek premium kupowanych przez podobnych użytkowników. Apriori odkrywa, że produkt jest często kupowany razem z etui ochronnym (lift=2.1). Wszystkie trzy metody działają równolegle, każda dostarczając innej perspektywy.

\textbf{Różnorodność perspektyw - trójwymiarowy obraz produktów}

System dostarcza użytkownikowi trzech fundamentalnie odmiennych perspektyw na produkty, co znacząco wzbogaca proces decyzyjny:

\begin{itemize}
\item \textbf{Collaborative Filtering:} ,,Użytkownicy o podobnych preferencjach do Twoich kupowali również...'' - perspektywa społeczna oparta na zachowaniach grupy podobnych użytkowników,
\item \textbf{Analiza sentymentu:} ,,Produkty najlepiej oceniane przez wszystkich użytkowników i ekspertów to...'' - perspektywa jakościowa oparta na agregacji opinii i opisów,
\item \textbf{Apriori:} ,,Produkty często kupowane razem z tym, który wybrałeś to...'' - perspektywa funkcjonalna oparta na faktycznych wzorcach użytkowania.
\end{itemize}

Ta wielowymiarowość eliminuje jednostronność - użytkownik nie jest ograniczony do jednej metody rekomendacji, lecz otrzymuje kompletny obraz uwzględniający społeczne wzorce zakupowe, obiektywną jakość oraz funkcjonalną komplementarność produktów.

\textbf{Weryfikacja krzyżowa jakości rekomendacji}

System umożliwia wzajemną weryfikację rekomendacji między metodami, co służy jako mechanizm kontroli jakości. Scenariusze weryfikacji:

\textit{Scenariusz 1 - Detekcja produktów niskiej jakości w CF:} Collaborative Filtering sugeruje produkt X (wysokie podobieństwo 0.85), ale analiza sentymentu pokazuje wynik -0.3 (negatywny). Może to wskazywać na problem - produkt kupowany z powodu promocji lub błędnego zamówienia, a nie rzeczywistej jakości. Administrator powinien zbadać przyczynę anomalii.

\textit{Scenariusz 2 - Identyfikacja nisz rynkowych:} Analiza sentymentu wskazuje produkt Y o bardzo wysokim sentymencie (0.72), ale Collaborative Filtering nie generuje dla niego rekomendacji (niska częstość zakupów). To może wskazywać na niszowy produkt wysokiej jakości, który nie jest szeroko znany - potencjalna okazja do promocji.

\textit{Scenariusz 3 - Walidacja reguł Apriori:} Apriori sugeruje regułę ,,produkt A → produkt B'' z wysokim lift=2.5. Sprawdzenie sentymentu produktu B pokazuje wynik 0.65 (pozytywny), co potwierdza, że reguła wskazuje na wartościowy produkt. Gdyby sentyment był negatywny, mogłoby to sugerować, że produkt B jest kupowany z konieczności (np. naprawy, zamienniki), a nie jako wartościowa rekomendacja.

\subsection*{7.3 Porównanie aspektów wydajnościowych}

\textbf{Złożoność obliczeniowa i czas generowania rekomendacji}

\textit{Collaborative Filtering:} Złożoność $O(n^2 \cdot u)$ gdzie $n$ to liczba produktów, $u$ to liczba użytkowników. Dla katalogu 1000 produktów i 500 użytkowników wymagane jest $500 \times 10^6$ porównań dla konstrukcji pełnej macierzy podobieństw. System wykorzystuje prekalkulację - macierz jest obliczana cyklicznie (co 2 godziny) i zapisywana w tabeli \texttt{method\_product\_similarity} z indeksami na kolumnach \texttt{product1\_id} i \texttt{product2\_id}. Dzięki temu zapytanie użytkownika o rekomendacje wykonuje jedynie odczyt z bazy (czas odpowiedzi <50ms), nie wymaga obliczeń w czasie rzeczywistym. Wąskim gardłem jest przeliczanie macierzy dla dużych katalogów - dla 10,000 produktów czas przeliczenia wynosi około 15-20 minut.

\textit{Analiza sentymentu:} Złożoność $O(p \cdot t)$ gdzie $p$ to liczba produktów, $t$ to średnia długość tekstów. Proces analizy dla 1000 produktów z 5 źródłami tekstowymi (średnio 200 słów każde) wymaga przetworzenia $1000 \times 5 \times 200 = 1,000,000$ tokenów. System wykorzystuje Django Signals - sentyment jest aktualizowany automatycznie po dodaniu opinii lub zmianie opisu produktu (inkrementalna aktualizacja), nie wymaga pełnego przeliczenia katalogu. Czas odpowiedzi zapytania: <30ms (odczyt z tabeli \texttt{method\_product\_sentiment\_summary}).

\textit{Apriori:} Złożoność $O(t \cdot n^2)$ gdzie $t$ to liczba transakcji, $n$ to liczba produktów po przycinaniu. System stosuje bitmap pruning - transakcje reprezentowane są jako liczby całkowite z ustawionymi bitami odpowiadającymi produktom, co umożliwia sprawdzenie współwystępowania jedną operacją bitową AND zamiast iteracji po liście. Wczesne przycinanie eliminuje produkty o support <0.1\%, co redukuje $n$ o około 80-90\%. Dla 5000 transakcji i 1000 produktów (po przycinaniu 150 produktów) czas generowania reguł wynosi około 2-3 minuty. Operacje zbiorcze bulk insert (partia 500 rekordów) znacznie przyspieszają zapis do tabeli \texttt{method\_productassociation}.

\textbf{Zużycie pamięci i skalowanie}

\textit{Collaborative Filtering:} Macierz podobieństw dla $n$ produktów wymaga $n \times (n-1) / 2$ rekordów (macierz symetryczna, bez diagonali). Dla 1000 produktów: około 500,000 rekordów w bazie, około 20MB pamięci. Dla 10,000 produktów: 50 milionów rekordów, około 2GB. Skalowanie kwadratowe jest głównym ograniczeniem metody.

\textit{Analiza sentymentu:} Jeden rekord per produkt w tabeli agregacji (5 źródeł + wynik końcowy). Dla 10,000 produktów: 10,000 rekordów, około 500KB pamięci. Skalowanie liniowe, brak problemów wydajnościowych.

\textit{Apriori:} Liczba reguł zależy od progów (support, confidence, lift). Dla typowych progów (support>=0.1\%, confidence>=5\%, lift>=1.0) generowanych jest około 500-2000 reguł dla katalogu 1000 produktów. Pamięć: około 100-500KB. Skalowanie zależy od gęstości grafu transakcji - jeśli użytkownicy kupują bardzo różnorodne kombinacje produktów, liczba reguł rośnie szybciej.

\textbf{Podsumowanie wydajności}

Dla typowych rozmiarów katalogów e-commerce (1,000-5,000 produktów) wszystkie trzy metody działają z akceptowalnymi czasami odpowiedzi dzięki zastosowanym optymalizacjom. Collaborative Filtering jest najbardziej wymagającą metodą (skalowanie $O(n^2)$), ale prekalkulacja eliminuje problem w kontekście zapytań użytkowników. Apriori wymaga najmniej pamięci i skaluje się najlepiej. Analiza sentymentu jest najbardziej efektywna pod względem czasu i pamięci.

\subsection*{7.4 Obserwacje dotyczące stabilności i pokrycia katalogu}

\textbf{Stabilność rekomendacji w czasie}

\textit{Collaborative Filtering:} Rekomendacje charakteryzują się wysoką stabilnością dla użytkowników z bogatą historią zakupów (>10 transakcji). Dodanie jednej nowej transakcji nieznacznie modyfikuje macierz podobieństw - zmienia się tylko sąsiedztwo produktów z nowej transakcji. Przykład: użytkownik z 15 zakupami dodaje 16. zakup (nowe słuchawki). Jego lista rekomendowanych produktów zmienia się o około 10-15\% (2-3 nowe produkty wśród top 20), co zapewnia spójność doświadczeń - użytkownik nie jest dezorientowany całkowicie odmienną listą sugestii. Stabilność jest kluczowa dla budowania zaufania użytkownika do systemu.

\textit{Analiza sentymentu:} Znacznie bardziej dynamiczna. Każda nowa opinia (waga 40\%) wpływa natychmiast na pozycję produktu w rankingu. Przykład: produkt z 50 pozytywnymi opiniami (sentyment 0.65) otrzymuje 5 negatywnych opinii - sentyment spada do 0.52. Produkt schodzi z top pozycji w wyszukiwarce. Ta dynamika odzwierciedla aktualny konsensus społeczny i chroni użytkowników przed produktami, których jakość się pogorszyła.

\textit{Apriori:} Stabilność średnia. Nowe transakcje inkrementalnie zwiększają support istniejących reguł lub tworzą nowe reguły dla nowych kombinacji produktów. Reguły z bardzo wysokim lift (>3.0) są stabilne - wymagałoby bardzo wielu nietypowych transakcji, aby je zdezawansować.

\textbf{Pokrycie katalogu i problem długiego ogona}

Analiza pokrycia pokazuje fundamentalną różnicę między metodami:

\textit{Collaborative Filtering:} Generuje rekomendacje dla około 75\% produktów w katalogu. Wymóg: produkt musi występować w co najmniej 2 transakcjach z innymi produktami, aby móc obliczyć podobieństwo. 25\% produktów (typowo nowe lub niszowe) pozostaje bez rekomendacji CF.

\textit{Apriori:} Pokrywa około 60\% produktów. Wymóg: produkt musi występować w transakcjach wieloproduktowych (pojedyncze zakupy nie tworzą reguł) z wystarczającym supportem (>=0.1\%). 40\% produktów nie ma reguł asocjacyjnych.

\textit{Analiza sentymentu:} Pokrywa 100\% katalogu bez wyjątków. Każdy produkt ma opis i nazwę (minimum 2 źródła tekstowe, łączna waga 40\%), co wystarcza do wygenerowania oceny sentymentu. To kluczowa przewaga - żaden produkt nie jest ,,niewidoczny'' w systemie.

Komplementarność metod zapewnia, że każdy produkt jest dostępny przynajmniej w jednym mechanizmie rekomendacji. Nowe produkty bez historii (25\% katalogu niedostępne w CF, 40\% w Apriori) są w pełni reprezentowane w analizie sentymentu (100\%).

\textbf{Serendipity - odkrywanie niespodziewanych ale trafnych produktów}

\textit{Collaborative Filtering:} Wykazuje najwyższą zdolność do generowania rekomendacji serendipicznych. Algorytm odkrywa nieoczywiste powiązania wynikające ze złożonych, wielopoziomowych wzorców zakupowych dużej grupy użytkowników. Przykład rzeczywisty: użytkownik kupujący sprzęt fotograficzny (aparat, obiektywy) otrzymuje rekomendację plecaka turystycznego. Powiązanie: inni użytkownicy kupujący sprzęt fotograficzny często są fotografami krajobrazowymi lub podróżniczymi, którzy potrzebują plecaków do transportu sprzętu w teren. Powiązanie nie jest oczywiste z kategorii produktów (,,Fotografia'' vs ,,Turystyka''), ale jest merytorycznie trafne i wartościowe.

\textit{Apriori:} Niska serendipity, wysoka precyzja. Reguły wskazują produkty funkcjonalnie komplementarne, które użytkownik mógłby łatwo przewidzieć (kamera → karta pamięci, laptop → mysz). Wartość leży nie w zaskoczeniu, ale w przypomnieniu o koniecznych akcesoriach.

\textit{Analiza sentymentu:} Brak serendipity. Prezentuje uniwersalnie najlepiej ocenione produkty, które są popularne i dobrze znane. Nie odkrywa ukrytych powiązań, ale gwarantuje wysoką jakość sugerowanych produktów.

\subsection*{7.5 Synteza - wartość zintegrowanego systemu}

Żadna z metod nie jest uniwersalnie najlepsza - każda wykazuje przewagę w określonym kontekście użycia. Kluczowe wnioski z porównania:

\textbf{Dominacja w specyficznych kontekstach:}
\begin{itemize}
\item \textbf{Collaborative Filtering} - najlepsza dla eksploracji katalogu i personalizacji dla użytkowników z historią zakupów, najwyższa serendipity,
\item \textbf{Analiza sentymentu} - fundament oceny jakości, jedyna metoda z 100\% pokryciem katalogu, całkowite rozwiązanie problemu zimnego startu,
\item \textbf{Apriori} - bezkonkurencyjna precyzja w cross-sellingu i sugerowaniu produktów komplementarnych, najwyższa trafność w koszyku zakupowym.
\end{itemize}

\textbf{Synergiczne działanie metod:}

Wartość zintegrowanego systemu przekracza sumę wartości pojedynczych komponentów. Użytkownik w różnych fazach ścieżki zakupowej otrzymuje wsparcie najbardziej adekwatnej metody:

\begin{enumerate}
\item \textit{Faza eksploracji} (strona główna) - Collaborative Filtering odkrywa nieoczywiste powiązania i sugeruje produkty poza obszarem bieżących zainteresowań,
\item \textit{Faza oceny} (wyszukiwarka) - Analiza sentymentu filtruje produkty według jakości, eliminując produkty o negatywnych opiniach,
\item \textit{Faza finalizacji} (koszyk) - Apriori przypomina o koniecznych akcesoriach i produktach komplementarnych, zwiększając wartość koszyka.
\end{enumerate}

Ta wielowymiarowość tworzy kompleksowe wsparcie procesu decyzyjnego, znacząco przewyższające możliwości każdej metody działającej osobno. System dostarcza użytkownikowi holistycznego obrazu produktów uwzględniającego społeczne wzorce zakupowe, obiektywną jakość oraz funkcjonalną komplementarność.
...
\bibitem{attachment2026}
Dawid Olko, Piotr Smola,
\textit{Załącznik A: Szczegółowe scenariusze użycia systemu SmartRecommender},
Materiał pomocniczy do pracy inżynierskiej, 2026.
Dostępny online: \url{https://github.com/dawidolko/SmartRecommender-Project-Django-React/blob/main/.docs/latex/olko/attachment.pdf}
