...
\usepackage[lf]{carlito} % Czcionka podobna do Calibri, lf = lining figures (cyfry wyrównane)
\renewcommand{\familydefault}{\sfdefault} % Ustawienie Carlito jako domyślnej czcionki
...
\begin{itemize}
\item \textbf{Wysokie koszty operacyjne} - rozwiązania chmurowe wiążą się z regularnymi opłatami licencyjnymi, które mogą być znaczące dla małych i średnich platform e-com\-merce. 
Dla porównania, własna implementacja eliminuje te koszty przy zachowaniu kontroli nad funkcjonalnościami,
\item \textbf{Brak natywnej analizy sentymentu} - Amazon Personalize koncentruje się wyłącznie na collaborative filtering i nie oferuje analizy opinii produktów. Wieloźródłowa agregacja sentymentu (opinie + opis + nazwa + specyfikacje + kategorie) zastosowana w niniejszej pracy wymaga integracji z AWS lub samodzielnej implementacji,
...
Dla porównania: migracja z Amazon Personalize do Google Recommendations AI wymaga przepisania całej integracji (śledzenie zdarzeń, dane treningowe, wywołania API) oraz ponownego trenowania modeli, co może trwać dłuższą chwilę i powodować degradację jakości rekomendacji.
\end{itemize}
...
\item Statystyki produktów (najczęściej kupowane).
\end{itemize}
...
Baza danych systemu składa się z 25 tabel zorganizowanych w cztery moduły funkcjonalne. Poniższe diagramy ERD (Entity-Relationship Diagram) przedstawiają strukturę relacyjną bazy danych wraz z kluczowymi powiązaniami między tabelami.
...
\begin{itemize}
\item \texttt{method\_\allowbreak product\_\allowbreak similarity} - macierz podobieństw produktów (Collaborative Filtering),
...
\begin{itemize}
\item \texttt{method\_\allowbreak product\_\allowbreak similarity} - macierz podobieństw Collaborative Filtering,
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/recomendationSystemAdmin4.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/opinionView.jpg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{images/associationAdmin.png}
...
Rysunek \ref{fig:association_debug_1} przedstawia pierwszy ekran panelu debugowania algorytmu Apriori. Pod numerem (1) administrator wybiera produkt z listy rozwijanej. Poniżej pod numerem (2) system wyświetla dane produktu z bazy danych, obliczenia transakcji oraz wszystkie reguły asocjacyjne dla wybranego produktu.
...
\textbf{(3) Optymalizacja jest kluczowa dla wydajności.} Bitmap pruning w Apriori znacząco przyspiesza przetwarzanie -- reprezentacja transakcji jako liczb binarnych. Cache'owanie wyników Sentiment eliminuje powtarzalne obliczenia. Indeksowanie w bazie danych (model \texttt{ProductSimilarity}) przyspiesza wyszukiwanie podobnych produktów.
...
Niniejsza praca przedstawiła proces implementacji oraz analizy kompletnego systemu e-commerce wyposażonego w mechanizmy rekomendacji produktów. System został opracowany od podstaw we współpracy dwuosobowej, przy czym w ramach pracy zaimplementowano trzy metody rekomendacyjne: Collaborative Filtering z metryką Adjusted Cosine Similarity, analizę sentymentu opartą na podejściu słownikowym oraz reguły asocjacyjne wykorzystujące algorytm Apriori.
...
Przeprowadzona analiza porównawcza sześciu metod rekomendacyjnych wykazała, że każda z nich charakteryzuje się specyficznymi zaletami i ograniczeniami. Collaborative Filtering skutecznie wykrywa nieoczywiste powiązania międzykategorialne, lecz wymaga bogatej historii transakcyjnej. Analiza sentymentu rozwiązuje problem zimnego startu poprzez wieloźródłową agregację tekstu, natomiast algorytm Apriori z optymalizacją bitmap pruning efektywnie generuje reguły współzakupowe dla strategii cross-sellingu.
